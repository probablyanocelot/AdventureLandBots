...
character.on("loot", function (data) {
  ...
  // ],
  //   }
  if (locate_item("computer") == -1) return;
  if (!data.items) return;
  for (let item of data.items) {
    if (item.looter && item.looter != character.name) continue;
    if (sell_dict["farmerSell"].includes(item.name)) {
      let idxItemToSell = locate_item(item.name);
      if (idxItemToSell == -1) continue;
      let itemToSell = character.items[idxItemToSell];
      if (itemToSell.hasOwnProperty("level") && itemToSell.level > 0) continue;
      sell(idxItemToSell);
      // send_tg_bot_message(`FARMER SELLING ${item.name}, slot ${idxItemToSell}`);
    }
  }
});
...
function farmerBank() {
  ...
  if (character.bank) {
    // && character.bank['items0'].length < 42
...
class Farmer {
  ...
  handle_death() {
    if (character.rip) {
      ...
      if (
        this.last_respawn == null ||
        new Date() - this.last_respawn >= 10000
      ) {
        this.clear_current_action();
        respawn();
  ...
  async wabbitHunt() {
    if (!parent.S.wabbit.live && this.current_action == "wabbit")
      this.clear_current_action();
  ...
  async serverMiniEvents() {
    ...
    for (let boss of bosses) {
      if (parent.S[boss] && parent.S[boss].live) {
        if (
          boss == "snowman" &&
          (parent.S?.mrgreen?.live || parent.S?.mrpumpkin?.live)
        )
          return;
        ...
        if (
          !this.current_action ||
          (this.current_action && !mobsGroup.includes(this.current_action))
        )
    ...
    smart_move(loc)
      .catch(() => {
        if (!character.moving && !smart.moving)
          use_skill("use_town").then(smart_move(loc));
      })
      .then(then);
  }
  ...
  manage_healing() {
    if (character.ctype == "paladin") {
      if (!is_on_cooldown("regen_mp") && character.mp < character.max_mp - 500)
      ...
      if (
        ...
        if (character.mp > 100) {
          use_skill("selfheal", character).then(
            (s) => {
              return;
            },
            (f) => {
      ...
      if (!is_on_cooldown("regen_mp") && character.mp < character.max_mp - 500)
        use(locate_item(desired_mp_pot));
      if (!is_on_cooldown("regen_hp") && character.hp <= character.max_hp - 400)
        use(locate_item(desired_hp_pot));
    }
    if (!is_on_cooldown("regen_mp") && character.mp < character.max_mp)
      use_skill("regen_mp");
  ...
  async handleMonsterHunt() {
    ...
    if (character.s.monsterhunt) {
      ...
      if (character.s.monsterhunt.c > 0) {
        ...
        if (
          !mobsLow.includes(mobType) &&
          this.current_action == "monsterhunt"
        ) {
          this.clear_current_action();
          this.can_kill = false;
          return;
        ...
        if (
          this.current_action == "farming" &&
          character.x != monsterHunterLocation.x &&
          character.y != monsterHunterLocation.y
        )
          return;

        // if can kill or not current action, set
        this.can_kill = true;
        if (this.current_action != "monsterhunt")
  ...
  manage_combat(lastTargetID) {
    ...
    if (
      !target &&
      this.current_action &&
      G.monsters.hasOwnProperty(this.current_action)
    )
    ...
    if (!mobsFocus.includes(targetName)) {
      if (get_nearest_monster({ type: "phoenix" }))
    ...
    switch (character.ctype) {
      case "mage":
        if (!member.s.energize && !is_on_cooldown("energize"))
          use_skill("energize");
        break;

      case "ranger":
        if (
          target.max_hp > character.attack * 3 &&
          !is_on_cooldown("huntersmark") &&
          character.mp >= 400
        )
          use_skill("huntersmark", target);
        if (
          target.max_hp >= character.attack * 1.5 &&
        ...
        if (
          target.max_hp < character.attack * 0.7 * 3 * 2 &&
      ...
      case "priest":
        ...
        if (
          ...
          character.max_hp - character.hp >= character.heal
        )
          use_skill("heal", character.id);
        ...
        if (
          ...
          is_friendly(target.target) &&
          target.target != character.name &&
          !mobsGroup.includes(target.name) &&
      ...
      case "rogue":
        ...
        if (
          !is_on_cooldown("quickstab") &&
          character.mp > G.skills.quickstab.mp
        )
      ...
      case "warrior":
        ...
        let wepType = (hand) => {
          return G.items[character.slots[hand].name].wtype;
        }; // checks weapon type

        if (!is_on_cooldown("charge")) use_skill("charge");
        if (
          war2h.includes(wepType("mainhand")) &&
          !is_on_cooldown("cleave") &&
          target.attack <= 500 &&
          character.mp >= G.skills.cleave.mp
        )
          use_skill("cleave").then(send_cm("prayerBeads", { cmd: "absorb" }));
    ...
    if (!is_on_cooldown("attack")) {
      if (
        character.ctype == "priest" &&
        character.max_hp - character.hp >= character.heal * 0.8
      )
        use_skill("heal", character.id);
      attack(target);
    }
  ...
  party_buff(skill, currentGroup) {
    ...
    for (let member of party) {
      ...
      if (!member.s[skill] && !is_on_cooldown(skill))
        use_skill(skill, member.id);
    }
  }
  //}
...
function item_quantity(name) {
  ...
  for (var i = 0; i < 42; i++) {
    if (character.items[i] && character.items[i].name == name)
      itemCount += character.items[i].q || 0;
  }
  return itemCount;
}
...
function getCompanionTarget(companionName) {
  ...
  if (!companionMonster || companionMonster.rip || companionMonster.dead)
    return false;
...
function combatDistancing(target) {
  if (target) {
    ...
    if (
      mobLocationDict.hasOwnProperty(target.mtype) &&
    ...
    if (
      target.hp > character.attack * 2 &&
      distanceToTarget(target) <= character.range * 0.25
    ) {
...
function equipFromItems(itemName, invSlot, gearSlotName) {
  ...
  if (isInItems(invSlot) && (!equippedItem || equippedItem.name != itemName))
    equip(invSlot);
}
...
function doWeaponEquip(itemName, hand = "mainhand") {
  let itemSlot = locate_item(itemName);
  if (character.items[itemSlot].name != itemName)
