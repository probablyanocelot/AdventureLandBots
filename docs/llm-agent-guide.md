...
| `server-events` | `lib/services/server-events/index.js` | `joinFirstActiveEventService`, `runJoinEventModuleService`, `broadcastCodeLoadedService`, `isJoinableEventService`, `getActiveJoinableEventsService`, `createEventTaskEmitter` | module installer currently no args for join flow                                             | join-event call validates `{ ok: boolean }`; listeners support install/stop lifecycle  |
| `farming`       | `lib/services/farming/index.js`       | `createFarmingModuleService`, `createNoEventFarmingModuleService`, `createRoleSyncRequesterService` (+ re-exported farming helpers)                                            | module install ctx: `{ cfg, runtimeScope }`; role sync service: `{ cfg, ownerName, reason }` | farming and role-sync services validate `stopRoutine()`                                |
