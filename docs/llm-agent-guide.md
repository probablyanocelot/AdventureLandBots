...
| `farming`       | `lib/services/farming/index.js`       | `createFarmingModuleService`, `createRoleSyncRequesterService`, `getFarmingConfig` (+ re-exported farming helpers)                                                             | module install ctx: `{ cfg, runtimeScope }`; role sync service: `{ cfg, ownerName, reason }` | farming and role-sync services validate `stopRoutine()`                                |
