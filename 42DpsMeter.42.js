...
function updateTimerList() {
  ...
  let listString =
    '<table style="border-style: solid;" border="5px" bgcolor="black" align="right" cellpadding="5"><tr align="center"><td colspan="2">Damage Meter</td></tr><tr align="center"><td>Name</td><td>DPS</td></tr>';
...
function getDPS(partyMember) {
  ...
  if (entry != null) {
    ...
    dps = parseFloat(
      Math.round((entry.sumDamage / (elapsed / 1000)) * 100) / 100,
    ).toFixed(2);
  }
  return dps;
}
...
function getTotalDPS() {
  ...
  if (minStart != null) {
    ...
    dps = parseFloat(
      Math.round((sumDamage / (elapsed / 1000)) * 100) / 100,
    ).toFixed(2);
  }
...
function dpsmeterHitHandler(event) {
  if (parent != null) {
    ...
    let attackerEntity = parent.entities && parent.entities[attacker];

    if (attacker == character.name) {
      attackerEntity = character;
    }

    // attackerEntity can be undefined if the attacker isn't currently in view / not in parent.entities
    // Count DPS only for self or same-party members
    let isSelf = attacker == character.name;
    let isSameParty =
      character.party != null &&
      attackerEntity != null &&
      attackerEntity.party == character.party;
    if (isSelf || isSameParty) {
