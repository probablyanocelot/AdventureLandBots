...
async function main() {
  ...
  if (!BotCharacterCtor) {
    const botLib = await require("./characters/bot.js");
    BotCharacterCtor = botLib.BotCharacter;
  }

  const { getConfig, getRuntimeContext } = await require("./config.js");
  const cfg = getConfig();
  ...
  if (!runtime.inIframe && typeof performance_trick === "function") {
    try {
      ...
      console.log("Failed to run performance_trick", e);
    }
  }
  ...
  const startEventCombatIfEnabled = async () => {
    try {
      ...
      const { installEventCombat } =
        await require("./farming/combat/event_combat.js");
  ...
  const startSustainIfEnabled = async () => {
    try {
      const { installSustain } = await require("./routines/sustain.js");
      installSustain({ cfg });
    } catch (e) {
      console.log("Failed to start sustain routine", e);
    }
  };
  ...
  const joinActiveEventIfAvailable = async () => {
    try {
      ...
      const { isJoinableEvent } = await require("./farming/server_events.js");
  ...
  const startNoEventFarmingIfEnabled = async () => {
    try {
      ...
      const { installNoEventFarming } =
        await require("./farming/no_event_farming.js");
