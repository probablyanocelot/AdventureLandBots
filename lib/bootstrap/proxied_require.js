...
const createProxiedRequire = () => {
  const DEFAULT_WEB_BASE =
    "https://raw.githubusercontent.com/probablyanocelot/AdventureLandBots/refs/heads/dev-stable";
  const explicitWebBase =
    (typeof window !== "undefined" &&
      ((window.AL_BOTS_LOCAL_LOADING?.enabled &&
        window.LOCAL_CODES_HTTP_BASE) ||
        window.RAW_BASE ||
        "")) ||
    "";
  const WEB_BASE =
    (explicitWebBase.trim().replace(/\/+$/, "") || DEFAULT_WEB_BASE) + "/";
  const FOLDER = "lib/";
  ...
  const reconcileLocalLoading = () => {
    if (typeof window === "undefined") return;

    const explicitBaseDir =
      asTrimmedNonEmptyString(
        window.LOCAL_CODES_LOCATION ||
          window.parent?.LOCAL_CODES_LOCATION ||
          "",
      ) || null;

    if (!window.AL_BOTS_LOCAL_LOADING && explicitBaseDir) {
      window.AL_BOTS_LOCAL_LOADING = {
        enabled: true,
        fallbackToRemote: true,
        baseDir: stripTrailingPathSeparators(explicitBaseDir),
      };
    }
  };

  const getLocalLoadingConfig = () => {
    reconcileLocalLoading();
    const userCfg =
      (typeof window !== "undefined" && window.AL_BOTS_LOCAL_LOADING) || null;
    const globalLocalCodesBase =
      (typeof window !== "undefined" &&
        asTrimmedNonEmptyString(
    ...
    const configuredBaseDir = asTrimmedNonEmptyString(cfg.baseDir);
    cfg.baseDir = configuredBaseDir || globalLocalCodesBase;
    if (cfg.baseDir) {
      cfg.baseDir = stripTrailingPathSeparators(cfg.baseDir);
    }
  ...
  const shouldLogLoad = (fileName) => {
    const name = String(fileName || "");
    return (
      name.endsWith("al_main.js") ||
      name.endsWith("telemetry/client.js") ||
      name.endsWith("bootstrap/index.js")
    );
  };

  const logLoad = (source, fileName, detail) => {
    try {
      if (!shouldLogLoad(fileName)) return;
      const extra = detail ? ` (${detail})` : "";
      console.warn(`[ALBots loader] ${source} ${fileName}${extra}`);
    } catch {
  ...
  const fetchLocalModuleText = async (file_name, localCfg) => {
    const nodeRequire = getNodeRequire();
    if (!nodeRequire) {
      const err = new Error("Node require is unavailable");
      err.code = "ALBOTS_LOCAL_UNAVAILABLE";
      throw err;
    }
    ...
    if (!baseDir) {
      const err = new Error("Unable to resolve local AppData codes directory");
      err.code = "ALBOTS_LOCAL_UNAVAILABLE";
      throw err;
    }

    const absolutePath = path.join(baseDir, file_name);
    logLoad("local", file_name, absolutePath);
    ...
    if (localCfg.enabled) {
      try {
        ...
        const localUnavailable =
          e?.code === "ALBOTS_LOCAL_UNAVAILABLE" ||
          String(e?.message || "").includes("Node require is unavailable") ||
          String(e?.message || "").includes(
            "Unable to resolve local AppData codes directory",
          );

        if (!localCfg.fallbackToRemote && !localUnavailable) {
          const msg = e && e.message ? e.message : String(e);
          throw new Error(
        ...
        if (localUnavailable) {
          console.warn(
            `[ALBots loader] local loading unavailable for ${localLabel}; falling back to remote source.`,
          );
        }
      }
    }

    const attemptRemote = async (base) => {
      const url = base + file_name;
      logLoad("remote", file_name, url);
      return await fetchModuleText(url);
    };

    try {
      return await attemptRemote(WEB_BASE);
    } catch (remoteError) {
      const remoteMsg =
      ...
      if (WEB_BASE !== `${DEFAULT_WEB_BASE}/`) {
        try {
          console.warn(
            `[ALBots loader] remote base ${WEB_BASE} failed for ${file_name}; falling back to ${DEFAULT_WEB_BASE}/`,
          );
          return await attemptRemote(`${DEFAULT_WEB_BASE}/`);
        } catch (fallbackError) {
          const fallbackMsg =
          ...
          throw new Error(
            localError
              ? `proxied_require failed local (${localLabel}: ${localMsg}) and remote (${WEB_BASE}${file_name}: ${remoteMsg}) and fallback (${DEFAULT_WEB_BASE}/${file_name}: ${fallbackMsg})`
              : `proxied_require failed remote (${WEB_BASE}${file_name}: ${remoteMsg}) and fallback (${DEFAULT_WEB_BASE}/${file_name}: ${fallbackMsg})`,
          );
        }
      }
      ...
      throw new Error(
        localError
          ? `proxied_require failed local (${localLabel}: ${localMsg}) and remote (${WEB_BASE}${file_name}: ${remoteMsg})`
          : `proxied_require failed to fetch ${WEB_BASE}${file_name} (${remoteMsg})`,
      );
    }
  };
