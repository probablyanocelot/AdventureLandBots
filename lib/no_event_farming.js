...
const determineAssignment = ({ roster, meta, stats } = {}) => {
  ...
  const cfg = getConfig();
  const nf = cfg.noEventFarming || {};
  const tinycrabRangerName = nf.tinycrabRangerName || "camelCase";
  ...
  if (mode === "default") {
    if (available.ranger.length) {
      if (available.ranger.includes(tinycrabRangerName)) {
        tinycrab = [tinycrabRangerName];
      } else {
        tinycrab = [available.ranger[0]];
      }
    }

    const exclude = new Set(tinycrab);
  ...
  if (tinycrab.length + monsterhunt.length > 3) {
    monsterhunt = monsterhunt.slice(0, Math.max(0, 3 - tinycrab.length));
  }

  return { mode, tinycrab, monsterhunt };
};

const runTinycrab = ({ cfg } = {}) => {
  if (smart?.moving) return;
  const nowMs = now();
  if (cfg._lastTinyMove && nowMs - cfg._lastTinyMove < 5000) return;

  try {
    const target = get_nearest_monster?.({ type: "tinycrab" });
    if (target) {
...
const runMonsterhunt = ({ cfg } = {}) => {
  try {
    const target = getMonsterhuntTarget();
    if (!target) {
      const nowMs = now();
...
const installNoEventFarming = ({ cfg } = {}) => {
  ...
  const loop = () => {
    ...
    try {
      ...
      const isTiny = assignment.tinycrab?.includes(character.name);
      const isHunt = assignment.monsterhunt?.includes(character.name);

      if (isTiny) runTinycrab({ cfg });
      if (isHunt) runMonsterhunt({ cfg });
