...
const runMonsterhunt = ({ cfg, targetOverride } = {}) => {
  try {
    const target =
    ...
    try {
      ...
      if (character.name === leader) {
        ...
        if (worldEvent) {
          ...
          if (isHunt) runWorldEvent({ cfg, event: worldEvent });
      ...
      if (worldEvent && isHunt) {
        runWorldEvent({ cfg, event: worldEvent });
      } else {
        if (isTiny) runTinycrab({ cfg });
        if (isHunt) runMonsterhunt({ cfg, targetOverride: sharedTarget });
      }
      if (character.ctype === "mage") runMageSupport({ assigned: isHunt });
    } catch (e) {
