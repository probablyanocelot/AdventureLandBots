...
const { runCrab, runMonsterhunt, runWorldEvent, runMageSupport } =
  await require("./combat/no_event_combat.js");
const { onCharacter } = await require("../listeners.js");
const { timeNow } = await require("../timing/last.js");
const { createMoveManager } = await require("../moving/smart_move_tracking.js");
const { getActiveJoinableEvents, getActiveWorldEvents, pickWorldEvent } =
  await require("./server_events.js");
...
const determineAssignment = ({ roster, meta, stats } = {}) => {
  ...
  const cfg = getConfig();
  const nf = cfg.noEventFarming || {};
  const crabRangerName = nf.crabRangerName || "camelCase";
...
const installNoEventFarming = ({ cfg } = {}) => {
  ...
  const loop = async () => {
    ...
    try {
      ...
      const ttlOk = timeNow() - st.lastAssignmentAt < 12000;
