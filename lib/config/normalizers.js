...
const deepMerge = (base, override) => {
  if (!override || typeof override !== "object") return cloneDeep(base);
...
const normalizeConfig = (cfgInput) => {
  ...
  const farmingConfig = cfg.farming || {};
  cfg.farming = deepMerge(DEFAULT_CONFIG.farming, farmingConfig);
  ...
  if (!cfg.farming.crabRangerName && detectedRanger)
    cfg.farming.crabRangerName = detectedRanger;
  const crabJoinPolicyInput =
    cfg.farming.crabJoinPolicy && typeof cfg.farming.crabJoinPolicy === "object"
      ? cfg.farming.crabJoinPolicy
      : {};
  cfg.farming.crabJoinPolicy = {
    ...DEFAULT_CONFIG.farming.crabJoinPolicy,
    ...crabJoinPolicyInput,
  };
  cfg.farming.crabJoinPolicy.enabled =
    cfg.farming.crabJoinPolicy.enabled !== false;
  cfg.farming.crabJoinPolicy.minOnlineFarmers = Math.max(
    1,
    Number(
      cfg.farming.crabJoinPolicy.minOnlineFarmers ||
        DEFAULT_CONFIG.farming.crabJoinPolicy.minOnlineFarmers,
    ),
  );
  cfg.farming.crabJoinPolicy.requireCapableNonRangerPair =
    cfg.farming.crabJoinPolicy.requireCapableNonRangerPair !== false;
  cfg.farming.crabJoinPolicy.requireDifficultTarget = Boolean(
    cfg.farming.crabJoinPolicy.requireDifficultTarget,
  );
  cfg.farming.crabJoinPolicy.onlyTargets = Array.from(
    new Set(
      (Array.isArray(cfg.farming.crabJoinPolicy.onlyTargets)
        ? cfg.farming.crabJoinPolicy.onlyTargets
        : []
      )
  ...
  cfg.farming.crabJoinPolicy.denyTargets = Array.from(
    new Set(
      (Array.isArray(cfg.farming.crabJoinPolicy.denyTargets)
        ? cfg.farming.crabJoinPolicy.denyTargets
        : []
      )
        .map(normalizeMonsterName)
        .filter(Boolean),
  ...
  cfg.farming.manualFarmMob =
    typeof cfg.farming.manualFarmMob === "string" &&
    cfg.farming.manualFarmMob.trim()
      ? cfg.farming.manualFarmMob.trim().toLowerCase()
      : null;
  cfg.farming.manualFarmMobSelf =
    typeof cfg.farming.manualFarmMobSelf === "string" &&
    cfg.farming.manualFarmMobSelf.trim()
      ? cfg.farming.manualFarmMobSelf.trim().toLowerCase()
      : null;
  cfg.farming.crabHoldSelf =
    cfg.farming.crabHoldSelf == null ? null : Boolean(cfg.farming.crabHoldSelf);
  cfg.farming.disableHuntBlockers = cfg.farming.disableHuntBlockers === true;
  cfg.farming.requirePriestForWarriorHardPull =
    cfg.farming.requirePriestForWarriorHardPull !== false;
  cfg.farming.highestLuckRangerCrabHold =
    cfg.farming.highestLuckRangerCrabHold !== false;
  cfg.farming.highestLuckRangerRareMobRadius = Math.max(
    1,
    Number(
      cfg.farming.highestLuckRangerRareMobRadius ||
        DEFAULT_CONFIG.farming.highestLuckRangerRareMobRadius,
    ),
  );
  cfg.farming.highestLuckRangerEggLuckThresholdMs = Math.max(
    0,
    Number(
      cfg.farming.highestLuckRangerEggLuckThresholdMs ||
        DEFAULT_CONFIG.farming.highestLuckRangerEggLuckThresholdMs,
    ),
  );

  const chain =
    cfg.farming.aggroLockChain || DEFAULT_CONFIG.farming.aggroLockChain;
  cfg.farming.aggroLockChain = {
    ...DEFAULT_CONFIG.farming.aggroLockChain,
    ...(chain && typeof chain === "object" ? chain : {}),
  };
  cfg.farming.aggroLockChain.enabled = Boolean(
    cfg.farming.aggroLockChain.enabled,
  );
  cfg.farming.aggroLockChain.warriorName =
    cfg.farming.aggroLockChain.warriorName || null;
  cfg.farming.aggroLockChain.priestName =
    cfg.farming.aggroLockChain.priestName || null;
  cfg.farming.aggroLockChain.npcMageName =
    cfg.farming.aggroLockChain.npcMageName || null;
  cfg.farming.aggroLockChain.huntMageName =
    cfg.farming.aggroLockChain.huntMageName || null;
  cfg.farming.aggroLockChain.codeSlotOrName =
    cfg.farming.aggroLockChain.codeSlotOrName ?? null;
  cfg.farming.aggroLockChain.swapPriorityList = Array.isArray(
    cfg.farming.aggroLockChain.swapPriorityList,
  )
    ? cfg.farming.aggroLockChain.swapPriorityList.filter(Boolean)
    : [];
  cfg.farming.aggroLockChain.stopNpcMageAfterChainPort =
    cfg.farming.aggroLockChain.stopNpcMageAfterChainPort !== false;
  cfg.farming.aggroLockChain.requirePriestAggro =
    cfg.farming.aggroLockChain.requirePriestAggro !== false;
  cfg.farming.aggroLockChain.sameTargetOnly =
    cfg.farming.aggroLockChain.sameTargetOnly !== false;
  cfg.farming.aggroLockChain.requestCooldownMs = Math.max(
    1500,
    Number(
      cfg.farming.aggroLockChain.requestCooldownMs ||
        DEFAULT_CONFIG.farming.aggroLockChain.requestCooldownMs,
    ),
  );
  cfg.farming.aggroLockChain.pendingTimeoutMs = Math.max(
    3000,
    Number(
      cfg.farming.aggroLockChain.pendingTimeoutMs ||
        DEFAULT_CONFIG.farming.aggroLockChain.pendingTimeoutMs,
    ),
  );

  const chainNpcMage = cfg.farming.aggroLockChain.npcMageName || null;
  const chainHuntMage = cfg.farming.aggroLockChain.huntMageName || null;
  ...
  cfg.farming.huntMapDenyByTarget = normalizeTargetMapRules(
    cfg.farming.huntMapDenyByTarget,
    {
      fallback: DEFAULT_CONFIG.farming.huntMapDenyByTarget,
    },
  );
  cfg.farming.huntMapPreferByTarget = normalizeTargetMapRules(
    cfg.farming.huntMapPreferByTarget,
  );
  cfg.farming.huntMapAllowByTarget = normalizeTargetMapRules(
    cfg.farming.huntMapAllowByTarget,
  );
