...
class Merchant extends BotCharacter {
  ...
  async goGather(strGatherType) {
    ...
    // If we just finished channeling, unlock and rotate to the other task.
    ...
    // Wait out cooldown before attempting to move/equip/start this task.
    if (is_on_cooldown(task.type)) return;
    ...
    if (character.slots.mainhand?.name !== task.tool) {
      ...
      if (slot === -1) {
        // Missing required tool: drop the task and move on.
        this._gatherTask = null;
        this._gatherIndex = (this._gatherIndex + 1) % this._gatherOrder.length;
        return;
      }
