...
const DEFAULT_CONFIG = Object.freeze({
  ...
  // Optional fallback when no iframe child can be swapped out cleanly.
  // When enabled, the swap helper will call the named global hook with a
  // payload containing `route.href`, which can later do something like:
  //   parent.window.location.href = route.href;
  swapRouting: {
    enabled: false,
  ...
  noEventFarming: {
    ...
    // Optional manual override for the hunt target. Set to a monster type string
    // to prefer farming that mob until cleared back to null.
    manualFarmMob: null,
