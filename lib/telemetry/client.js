const { warn } = await require("../al_debug_log.js");
const { buildLocalWsUrl, getTelemetryWsPort } =
  await require("../al_env_config.js");

const st = {
  started: false,
  stopping: false,
  socket: null,
  reconnectTimer: null,
  publishTimer: null,
};

const sendTelemetryMessage = (payload) => {
  try {
    if (!payload || typeof payload !== "object") return false;
    if (!st.socket || st.socket.readyState !== WebSocket.OPEN) return false;
    st.socket.send(JSON.stringify(payload));
    return true;
  } catch {
    return false;
  }
};

const safeNumber = (value) => {
  const num = Number(value);
  return Number.isFinite(num) ? num : null;
};

const buildWsCandidates = (wsUrl) => {
  const candidates = [];
  const seen = new Set();

  const add = (value) => {
    if (!value || typeof value !== "string") return;
    if (seen.has(value)) return;
    seen.add(value);
    candidates.push(value);
  };

  add(wsUrl);

  try {
    const u = new URL(wsUrl);
    const isLoopbackHost =
      u.hostname === "localhost" ||
      u.hostname === "127.0.0.1" ||
      u.hostname === "::1";
    if (!isLoopbackHost) return candidates;

    const hostVariants = ["localhost", "127.0.0.1", "::1"];
    for (const host of hostVariants) {
      const copy = new URL(u.toString());
      copy.hostname = host;
      add(copy.toString());
    }
  } catch {
    // ignore parse failures, keep original URL only
  }

  return candidates;
};

const sanitizeItem = (item) => {
  if (!item || typeof item !== "object") return null;
  const out = { name: item.name };
  if (item.q !== undefined) out.q = item.q;
  if (item.level !== undefined) out.level = item.level;
  if (item.p !== undefined) out.p = item.p;
  if (item.l !== undefined) out.l = item.l;
  if (item.stat_type !== undefined) out.stat_type = item.stat_type;
  if (item.stat !== undefined) out.stat = item.stat;
  return out;
};

const buildPayload = () => {
  const items = Array.isArray(character?.items) ? character.items : [];
  return {
    type: "inventory:update",
    bot: {
      name: character?.name || null,
      ctype: character?.ctype || null,
      level: safeNumber(character?.level),
    },
    online: character ? !character.rip : true,
    location: {
      map: character?.map || null,
      in: character?.in || null,
      x: safeNumber(character?.x),
      y: safeNumber(character?.y),
    },
    gold: safeNumber(character?.gold),
    items: items.map(sanitizeItem),
    ts: Date.now(),
  };
};

const clearReconnectTimer = () => {
  try {
    if (st.reconnectTimer) clearTimeout(st.reconnectTimer);
  } catch {
    // ignore
  }
  st.reconnectTimer = null;
};

const clearPublishTimer = () => {
  try {
    if (st.publishTimer) clearInterval(st.publishTimer);
  } catch {
    // ignore
  }
  st.publishTimer = null;
};

const describeReadyState = (socket) => {
  const state = socket?.readyState;
  if (state === WebSocket.CONNECTING) return "CONNECTING";
  if (state === WebSocket.OPEN) return "OPEN";
  if (state === WebSocket.CLOSING) return "CLOSING";
  if (state === WebSocket.CLOSED) return "CLOSED";
  return String(state ?? "unknown");
};

const stringifyError = (err) => {
  try {
    if (!err) return "unknown error";
    if (typeof err === "string") return err;
    const code = err?.code ? `[${err.code}] ` : "";
    const msg = err?.message || String(err);
    return `${code}${msg}`;
  } catch {
    return "unknown error";
  }
};

const closeSocket = () => {
  try {
    st.socket?.close?.();
  } catch {
    // ignore
  }
  st.socket = null;
};

const stopTelemetry = () => {
  st.stopping = true;
  st.started = false;
  clearReconnectTimer();
  clearPublishTimer();
  closeSocket();
  st.stopping = false;
};

const buildDisposable = () => ({
  stop: () => {
    stopTelemetry();
  },
  dispose: () => {
    stopTelemetry();
  },
  [Symbol.dispose]: () => {
    stopTelemetry();
  },
  [Symbol.asyncDispose]: async () => {
    stopTelemetry();
  },
});

const bindSocketEvent = (socket, eventName, handler) => {
  if (!socket || typeof handler !== "function") return false;

  if (typeof socket.addEventListener === "function") {
    socket.addEventListener(eventName, handler);
    return true;
  }

  const key = `on${eventName}`;
  if (!(key in socket)) return false;

  const prev = socket[key];
  socket[key] = (...args) => {
    try {
      if (typeof prev === "function") prev.apply(socket, args);
    } catch {
      // ignore
    }
    handler(...args);
  };
  return true;
};

const connect = (wsUrl) => {
  const wsCandidates = buildWsCandidates(wsUrl);
  const failedCandidates = [];

  const connectIndex = (idx) => {
    const currentUrl = wsCandidates[idx];
    if (!currentUrl) return;

    let opened = false;
    let socket = null;
    try {
      socket = new WebSocket(currentUrl);
      st.socket = socket;
    } catch (e) {
      const detail = stringifyError(e);
      failedCandidates.push(`${currentUrl} -> create failed: ${detail}`);

      if (idx + 1 < wsCandidates.length) {
        warn(
          `Telemetry websocket failed to create (${currentUrl}) detail=${detail}; trying ${wsCandidates[idx + 1]}.`,
        );
        connectIndex(idx + 1);
        return;
      }

      warn(
        `Telemetry websocket failed to create (${currentUrl}) detail=${detail}. attempts=${failedCandidates.join(" | ")}`,
      );
      return;
    }

    const onOpen = () => {
      opened = true;
      try {
        socket?.send?.(JSON.stringify({ type: "client:hello", role: "bot" }));
      } catch {
        // ignore
      }
    };

    const onClose = (evt) => {
      if (st.socket === socket) st.socket = null;

      if (!opened && idx + 1 < wsCandidates.length) {
        failedCandidates.push(
          `${currentUrl} -> closed before open code=${evt?.code ?? "n/a"} reason=${evt?.reason || "n/a"}`,
        );
        warn(
          `Telemetry websocket connect failed (${currentUrl}); trying ${wsCandidates[idx + 1]}.`,
        );
        connectIndex(idx + 1);
        return;
      }

      if (!opened) {
        failedCandidates.push(
          `${currentUrl} -> closed before open code=${evt?.code ?? "n/a"} reason=${evt?.reason || "n/a"}`,
        );
        warn(
          `Telemetry websocket closed before open (${currentUrl}) code=${evt?.code ?? "n/a"} reason=${evt?.reason || "n/a"}. attempts=${failedCandidates.join(" | ")}`,
        );
      }

      if (!st.started || st.stopping) return;
      if (st.reconnectTimer) return;
      warn(
        `Telemetry websocket reconnect scheduled in 2000ms (${currentUrl}).`,
      );
      st.reconnectTimer = setTimeout(() => {
        st.reconnectTimer = null;
        if (!st.started || st.stopping) return;
        connect(wsUrl);
      }, 2000);
    };

    const onError = (evt) => {
      const detail =
        evt?.message || evt?.error?.message || evt?.type || "unknown error";
      failedCandidates.push(
        `${currentUrl} -> error state=${describeReadyState(socket)} detail=${detail}`,
      );
      warn(
        `Telemetry websocket error (${currentUrl}) state=${describeReadyState(socket)} detail=${detail}.`,
      );
      try {
        socket?.close?.();
      } catch {
        // ignore
      }
    };

    const wired =
      bindSocketEvent(socket, "open", onOpen) &&
      bindSocketEvent(socket, "close", onClose) &&
      bindSocketEvent(socket, "error", onError);

    if (!wired) {
      warn(
        "Telemetry disabled: WebSocket implementation lacks supported event hooks.",
      );
      try {
        socket?.close?.();
      } catch {
        // ignore
      }
      if (st.socket === socket) st.socket = null;
    }
  };

  connectIndex(0);
};

const startPublishing = (intervalMs) => {
  if (st.publishTimer) return;
  st.publishTimer = setInterval(
    () => {
      try {
        if (!st.socket || st.socket.readyState !== WebSocket.OPEN) return;
        const payload = buildPayload();
        if (!payload.bot?.name) return;
        st.socket.send(JSON.stringify(payload));
      } catch {
        // ignore
      }
    },
    Math.max(500, intervalMs),
  );
};

const installTelemetry = ({ cfg } = {}) => {
  if (st.started) return buildDisposable();
  st.started = true;

  const telemetryCfg = cfg?.telemetry || {};
  if (telemetryCfg.enabled === false) {
    st.started = false;
    return null;
  }

  if (typeof WebSocket === "undefined") {
    warn("Telemetry disabled: WebSocket is not available in this runtime.");
    st.started = false;
    return null;
  }

  const wsUrl = telemetryCfg.wsUrl || buildLocalWsUrl(getTelemetryWsPort());
  if (!wsUrl) {
    warn("Telemetry disabled: set TELEMETRY_WS_PORT or telemetry.wsUrl.");
    st.started = false;
    return null;
  }
  const wsCandidates = buildWsCandidates(wsUrl);
  warn(
    `Telemetry init: target websocket ${wsUrl}; candidates=${wsCandidates.join(", ")}.`,
  );
  const intervalMs = Number(telemetryCfg.intervalMs || 2000);

  connect(wsUrl);
  startPublishing(intervalMs);

  return buildDisposable();
};

module.exports = {
  installTelemetry,
  stopTelemetry,
  sendTelemetryMessage,
};
