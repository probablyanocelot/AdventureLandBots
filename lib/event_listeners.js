...
try {
  if (typeof character !== "undefined") {
    if (
      ...
      const originalSendCm = globalThis.send_cm.bind(globalThis);
      globalThis.send_cm = async (to, message) => {
        try {
          console.log("[CM->]", {
            to: safePreview(to),
        ...
        try {
          const result = await originalSendCm(to, message);
          try {
            console.log("[CM->ack]", safePreview(result));
          } catch {
            // ignore
          }
          return result;
          ...
          try {
            console.log("[CM->error]", String(e?.message || e));
          } catch {
            // ignore
          }
          throw e;
