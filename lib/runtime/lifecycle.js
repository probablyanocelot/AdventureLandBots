...
const createLifecycleScope = (label = "scope") => new LifecycleScope(label);

const runWithLifecycleScope = async (label, runFn) => {
  const scope = createLifecycleScope(label);
  try {
    return await runFn(scope);
  } finally {
