...
const upkeepModule = await require("../modules/upkeep.module.js");
const noEventFarmingModule =
  await require("../modules/no_event_farming.module.js");
const farmingModule = await require("../modules/farming.module.js");
const orchestratorModule = await require("../modules/orchestrator.module.js");

const MODULE_INSTALLERS = {
  telemetry: telemetryModule?.install,
  priestSwap: priestSwapModule?.install,
  party: partyModule?.install,
  joinEvent: joinEventModule?.install,
  unpackRequester: unpackRequesterModule?.install,
  eventCombat: eventCombatModule?.install,
  upkeep: upkeepModule?.install,
  farming: farmingModule?.install,
  noEventFarming: noEventFarmingModule?.install,
  orchestrator: orchestratorModule?.install,
};

const MODULE_POLICIES = {
  preBot: ["telemetry"],
  merchantPostBot: ["priestSwap", "party", "upkeep", "orchestrator"],
  nonMerchantPostBot: [
    "priestSwap",
    "party",
    "joinEvent",
