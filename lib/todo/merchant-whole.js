...
<<<<<<< HEAD:lib/todo/merchant-whole.js
let cfg = {
  runs: true,
};

let massexch = () => {
  if (!character.s?.massexchange) use_skill("massexchange");
  if (!character.s?.massexchangepp) use_skill("massexchangepp");
};
...
//load_code(53) // upgrade_all2 WILL ADD MORE
//performance_trick();
>>>>>>> 1d70c72381bbd5ba85cd3ee7ccf4d5399d21d054:3Merchant.3.js
...
class Merchant extends Character {
  constructor() {
    ...
    this._mluckWatched = new Set();
    this._mluckRefreshed = new Set();
    this._mluckRange = 320;

    // loops on init
    setInterval(this.regen, 500);
    setInterval(this.buff("mluck"), 1000);
    setInterval(() => this._applyMluckToNearbyPlayers(), 2000);
  }

  _applyMluckToNearbyPlayers() {
    if (typeof is_on_cooldown === "function" && is_on_cooldown("mluck")) return;
    if (!parent || !parent.entities || !character) return;

    const range = this._mluckRange;
    const nowNearby = new Set();

    for (const id in parent.entities) {
      const p = parent.entities[id];
      if (!p || p.type !== "character" || p.name === character.name) continue;

      const dist =
        typeof parent.distance === "function"
          ? parent.distance(character, p)
          : Infinity;
      if (dist > range) continue;

      nowNearby.add(p.name);
      const hasStrong = p.s?.mluck?.strong;
      const selfLuck = p.s?.mluck && p.s.mluck.f === character.name;
      const expiresSoon = selfLuck && p.s.mluck.ms <= 50 * 60 * 1000;

      if (expiresSoon) {
        if (!this._mluckRefreshed.has(p.name)) {
          try {
            use_skill("mluck", p);
          } catch {
            // ignore
          }
          this._mluckRefreshed.add(p.name);
        }
      } else {
        this._mluckRefreshed.delete(p.name);
      }

      if (!hasStrong || !p.s?.mluck) {
        // cast once when player enters range / when buff is missing
        if (!this._mluckWatched.has(p.name)) {
          try {
            use_skill("mluck", p);
          } catch {
            // ignore
          }
          this._mluckWatched.add(p.name);
        }
      } else {
        // strong mluck present, keep tracking for on-enter behavior
        this._mluckWatched.delete(p.name);
      }
    }

    // clear entries that walked out of range so re-apply on re-entry
    for (const name of this._mluckWatched) {
      if (!nowNearby.has(name)) {
        this._mluckWatched.delete(name);
      }
    }

    for (const name of this._mluckRefreshed) {
      if (!nowNearby.has(name)) {
        this._mluckRefreshed.delete(name);
      }
    }
  }
  ...
  loop() {
    ...
    if (character.rip) {
      ...
      if (this.idle_counter > 60 * 3 && cfg.runs) {
        this.do_runs();
      }
    }
  ...
  async buyFromGoblin2() {
    // use_skill("town")
<<<<<<< HEAD:lib/todo/merchant-whole.js
    if (smart.moving) return;
    this.set_current_action("buyFromGoblin2");
    ...
    await new Promise((resolve) => {
      ...
      parent.socket.once("lostandfound", (data) => {
        clearTimeout(timeout);

        if (Array.isArray(data) && data.length) {
          let anyBought = false;
          for (let d of data) {
            if (d.p && d.level === 0 && !sell_dict.merchSell.includes(d.name)) {
              parent.socket.emit("sbuy", { rid: d.rid, f: true }, d.q || 1);
              anyBought = true;
            }
            if (
              itemsToBuy.includes(d.name) &&
              !sell_dict.merchSell.includes(d.name)
            ) {
              if (d.level)
                parent.socket.emit("sbuy", { rid: d.rid, f: true }, 1);
              if (d.q) parent.socket.emit("sbuy", { rid: d.rid, f: true }, d.q);
              anyBought = true;
=======
    smart_move("woffice", () => {
...
>>>>>>> 1d70c72381bbd5ba85cd3ee7ccf4d5399d21d054:3Merchant.3.js
          ...
          if (!anyBought && data.length) {
            const least = data[0];
            parent.socket.emit(
              "sbuy",
              { rid: least.rid, f: true },
              least.q || 1,
            );
          }
        }
  ...
  async do_runs() {
    ...
    await smart_move({
      map: "cave",
      x: 1235.6287831244563,
      y: 22.294161428467266,
    });
    await smart_move(this.home)
      .then(async () => {
        buyFromPonty();
        if (!goblin_updated) {
          await this.buyFromGoblin2();
        } else {
          await smart_move("woffice")
            .then(async () => {
              buyFromGoblin();
  ...
  getEntityByName(name) {
    if (!name || !parent?.entities) return null;
    const targetName = String(name).toLowerCase();
    return Object.values(parent.entities).find(
      (e) => e && String(e.name || "").toLowerCase() === targetName,
    );
  }

  getEntityById(id) {
    if (!id || !parent?.entities) return null;
    const targetId = String(id).toLowerCase();
    return Object.values(parent.entities).find(
      (e) => e && String(e.id || "").toLowerCase() === targetId,
    );
  }

  distance(a, b) {
    if (!a || !b) return Infinity;
    const dx = Number(a.x || a.real_x || 0) - Number(b.x || b.real_x || 0);
    const dy = Number(a.y || a.real_y || 0) - Number(b.y || b.real_y || 0);
    return Math.hypot(dx, dy);
  }

  setCruiseSpeed(speed) {
    if (typeof cruise === "function") {
      try {
        cruise(speed);
      } catch {
        // ignore
  ...
  hasCrabAura(crabFarmer) {
    return Boolean(crabFarmer?.s?.citizen0aura?.ms > 0);
  }

  runCitizen0Lure() {
    return;
    const crabFarmerName = "camelCase"; // default crab ranger
    const crabFarmer = this.getEntityByName(crabFarmerName);
    if (!crabFarmer || crabFarmer.map !== character.map) {
      return;
    }

    const citizen0 =
      this.getEntityById("citizen0") || this.getEntityByName("Kane");
    const hasAura = this.hasCrabAura(crabFarmer);

    if (hasAura) {
      this._citizen0LureActive = false;
      this.setCruiseSpeed(500);
      return;
    }

    this._citizen0LureActive = true;
    this.setCruiseSpeed(30);

    const isNearby = citizen0 && this.distance(citizen0, crabFarmer) <= 150;

    if (!citizen0 || !isNearby) {
      // go find citizen0 if possible
      const goal = citizen0 || crabFarmer;
      if (goal && !smart.moving) {
        smart_move({ map: goal.map, x: goal.x, y: goal.y });
      }
    }

    const confettiSlot = locate_item("confetti");
    if (confettiSlot < 0) return;

    const now = Date.now();
    if (!this._lastConfettiAt || now - this._lastConfettiAt > 20000) {
      const northPoint = {
        map: crabFarmer.map,
        x: crabFarmer.x,
        y: crabFarmer.y - 100,
      ...
      try {
        throw_item(confettiSlot, northPoint.x, northPoint.y);
      } catch {
        // ignore
      }
      this._lastConfettiAt = now;
    }
  }
...
function upgrade_replacement() {
  ...
  for (let level = 0; level <= maxLevel; level++) {
    for (let itemIndex in character.items) {
      ...
      let itemName = item.name;
<<<<<<< HEAD:lib/todo/merchant-whole.js
      if (sell_dict.noUpgrade.includes(itemName)) continue;
      if (sell_dict.merchSell.includes(itemName)) continue;
=======
>>>>>>> 1d70c72381bbd5ba85cd3ee7ccf4d5399d21d054:3Merchant.3.js
      ...
      if (grade == 0) {
<<<<<<< HEAD:lib/todo/merchant-whole.js
        if (!item.p && !item.ps && sell_dict.merchSell.includes(itemName)) {
=======
