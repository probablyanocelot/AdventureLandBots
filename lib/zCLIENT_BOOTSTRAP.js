...
const proxied_require = (() => {
  ...
  const handler = async (file_name) => {
    ...
    const localUrl = file_name;
    const remoteUrl = WEB_BASE + file_name;
    let localError = null;

    if (localCfg.enabled) {
      try {
        return await fetchModuleText(localUrl);
      } catch (e) {
        localError = e;
        if (!localCfg.fallbackToRemote) {
          const msg = e && e.message ? e.message : String(e);
          throw new Error(
            `proxied_require local load failed for ${localUrl}: ${msg}`,
          );
        }
      }
    }
    ...
    try {
      ...
      throw new Error(
        localError
          ? `proxied_require failed local (${localUrl}: ${localMsg}) and remote (${remoteUrl}: ${remoteMsg})`
