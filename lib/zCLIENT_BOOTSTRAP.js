...
const resolveTelemetryWsUrl = () => {
  try {
    ...
    if (globalPort) return `ws://localhost:${globalPort}`;

    const localPort = readLocalTelemetryPort();
    if (localPort) return `ws://localhost:${localPort}`;

    if (
      typeof TELEMETRY_WS_PORT === "number" &&
      Number.isFinite(TELEMETRY_WS_PORT)
    ) {
      return `ws://localhost:${TELEMETRY_WS_PORT}`;
    }
  } catch {
    // ignore
  }
...
(async () => {
  try {
    if (typeof window !== "undefined" && !window.AL_BOTS_CONFIG) {
      ...
      window.AL_BOTS_CONFIG = telemetryWsUrl
        ? {
            telemetry: {
              enabled: true,
