...
// Local override for telemetry websocket URL when loading remote code.
// Set to a ws:// or wss:// URL (e.g., ws://127.0.0.1:8787) to bypass
// localhost-only port resolution.
const TELEMETRY_WS_URL = null;
...
const buildLocalTelemetryUrl = (port) =>
  port ? `ws://localhost:${port}` : null;
...
const resolveTelemetryWsUrl = () => {
  try {
    ...
    const constantUrl = asWsUrl(TELEMETRY_WS_URL);
    if (constantUrl) return constantUrl;

    const globalPort = asFiniteNumber(window?.TELEMETRY_WS_PORT);
    if (globalPort) return buildLocalTelemetryUrl(globalPort);

    const localPort = readLocalTelemetryPort();
    if (localPort) return buildLocalTelemetryUrl(localPort);

    if (
      typeof TELEMETRY_WS_PORT === "number" &&
      Number.isFinite(TELEMETRY_WS_PORT)
    ) {
      return buildLocalTelemetryUrl(TELEMETRY_WS_PORT);
    }
  } catch {
    // ignore
  }
