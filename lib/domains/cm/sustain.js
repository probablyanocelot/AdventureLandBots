...
// Cleanup: returns disposable (`stop` / `Symbol.dispose`) that stops the sustain loop.
...
const installSustain = ({ cfg } = {}) => {
  ...
  const trySustain = (kind) => {
    if (kind === "hp") {
      ...
      // Outside regen window, prefer MP sustain; avoid HP pots when priest support exists
      // unless we're in an emergency HP range.
      const priestEmergencyRatio = Math.max(
        0.1,
        Math.min(0.9, Number(cfg?.sustain?.priestHpPotEmergencyRatio ?? 0.45)),
      );
      const hpRatio =
  ...
  const loop = () => {
    ...
    try {
      ...
      // Prefer MP sustain actions first (especially with priest support).
      const usedMp = trySustain("mp");
      if (!usedMp) {
        trySustain("hp");
      }
    } catch {
