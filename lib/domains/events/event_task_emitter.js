...
// Inputs: game event announcements + `parent.S` liveness polling.
// Side effects: invokes supplied `onStart` / `onEnd` callbacks.
// Cleanup: `stopRoutine()` clears interval/subscriptions and active state.

const { info, warn } = await require("../../al_debug_log.js");
const { onGame } = await require("./listeners.js");
const { isJoinableEvent } = await require("./server_event_catalog.js");
