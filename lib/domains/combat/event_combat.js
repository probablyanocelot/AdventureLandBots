...
const getNearestActiveEventMonster = () => {
  try {
...
const pickTarget = (cfg) => {
    ...
    if (isInCrabxxEventContext() || hasVisibleCrabxx()) {
      ...
      if (!crabTarget) {
        dbg(
          ...
          "crabxx visible/context active, waiting for external aggro target",
          {
            in: character?.in,
            map: character?.map,
          },
  ...
  try {
    if (isInAbtestingContext()) {
      const enemyPlayer = getAbtestingEnemyTarget(cfg);
  ...
  try {
    const eventMob = getNearestActiveEventMonster();
    if (eventMob) return eventMob;
  } catch {
    // ignore
  }
