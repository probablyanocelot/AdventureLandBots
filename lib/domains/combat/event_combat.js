...
const isInJoinableEvent = () => {
  try {
    ...
    const inEvent = Boolean(character.in && isJoinableEvent(character.in));
...
const resolveTargetForEventName = (eventName, cfg) => {
  try {
    ...
    // Crab-style resilience: if event metadata is present but direct name
    // matching misses transiently, fall back to current/nearest visible monster.
    try {
      const current = get_targeted_monster?.();
...
const getNearestActiveEventTarget = (cfg) => {
  try {
    let active = getActiveJoinableEvents?.();
    if (!Array.isArray(active) || !active.length) return null;
...
const pickTarget = (cfg) => {
  ...
  // Extra crabxx fallback for inconsistent event state markers.
  try {
  ...
  // Extra abtesting fallback for inconsistent event state markers.
  try {
...
const tryAttack = (target, cfg) => {
  ...
  // Prefer engine's can_attack, but allow a resilient fallback for cases where
  // target lock is valid and we are in range while can_attack is flaky.
...
const installEventCombat = ({ cfg } = {}) => {
  ...
  const tick = async () => {
    ...
    try {
      ...
      // 0) Head to live event waypoint if available.
