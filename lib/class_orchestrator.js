...
const { onCharacter, waitForCm, waitForCmBatch } =
...
class Orchestrator {
  ...
  async _ensureMageRunning() {
    ...
    // Wait for mage to come online and emit a code-loaded CM.
    const start = Date.now();
    while (Date.now() - start < 30000) {
      try {
        const activeNames = getActiveNames();
        if (Array.isArray(activeNames) && activeNames.includes(mageName)) {
