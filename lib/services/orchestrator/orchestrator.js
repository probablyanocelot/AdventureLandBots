...
// Purpose: coordinate event participation, merchant assist/unpack flow, and mage swap-assisted porting.
// Inputs: global game runtime state + config (`getConfig`) + CM messages.
// Side effects: sends CM commands, moves/joins events, starts/swaps characters, sends items.
// Cleanup: `stopRoutine()` tears down listeners/timers/emitter; class is disposable.

const { getConfig } = await require("../../config/index.js");
const { info, warn } = await require("../../al_debug_log.js");
const { createEventTaskEmitter } = await require("../server-events/index.js");
const { onCharacter, waitForCmBatch } =
  await require("../runtime-listeners/index.js");
const { is_friendly, getActiveNames, ensureCharacterRunningBySwap } =
  await require("../party/index.js");
const { isGathering } = await require("../state/index.js");
const { sleepMs } = await require("../shared/index.js");
...
class Orchestrator {
  ...
  _installListeners() {
    if (this._offCm) return;

    // Listen for unpack requests from farmers.
    this._offCm = onCharacter("cm", (m) => {
  ...
  async _loop() {
    ...
    if (!this._running || this._disposed) return;
    this._loopTimer = setTimeout(() => this._loop(), 500);
  }
  ...
  async _handleUnpackRequest(req) {
    ...
    try {
      ...
      if (eventName && !this._merchantCanJoinEvent) {
        ...
        // Merchant can join or no event: move to farmer and signal arrival.
        if (isGathering()) {
          info(
            "Unpack request received while gathering; prioritizing unpack move",
            req.loc,
          );
        }
