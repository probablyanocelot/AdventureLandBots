  ...
  try {
    ...
    const policy =
      (nf?.crabJoinPolicy && typeof nf.crabJoinPolicy === "object"
        ? nf.crabJoinPolicy
        : getFarmingConfig(cfg).crabJoinPolicy) || {};
...
const selectWorldFarmers = ({ stats, available, skipRangerName } = {}) => {
  ...
  if (!trio.length) {
    const fallback = [
      ...(safeAvailable.priest || []),
      ...(safeAvailable.warrior || []),
      ...(safeAvailable.paladin || []),
      ...(safeAvailable.rogue || []),
      ...(safeAvailable.ranger || []),
      ...(safeAvailable.mage || []),
    ];
    trio = Array.from(new Set(fallback)).slice(0, 3);
  }
