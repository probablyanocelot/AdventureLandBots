...
const { getActiveWorldEvents, pickWorldEvent, isJoinableEvent } =
...
const ACTIVE_RANGER_EVENT_PAUSE_SET = new Set(["goobrawl"]);

const getActiveJoinableEventNames = () => {
  try {
    const active = parent?.S || {};
    return Object.keys(active).filter((name) => {
      if (!name) return false;
      if (!isJoinableEvent(name)) return false;
      const state = active[name];
      if (!state || typeof state !== "object") return false;
      if (typeof state.live === "boolean") return state.live;
      return true;
    });
  } catch {
    return [];
...
const shouldRangerBypassJoinableEvent = () => {
  try {
    if (String(character?.ctype || "").toLowerCase() !== "ranger") return false;
    const active = getActiveJoinableEventNames();
    if (!active.length) return false;
    return !active.some((name) => ACTIVE_RANGER_EVENT_PAUSE_SET.has(name));
  } catch {
    return false;
  }
};
...
const isInJoinableEventContext = () => {
  try {
    ...
    if (currentEvent && isJoinableEvent(currentEvent)) return true;
    return false;
  } catch {
    return false;
  }
...
const installNoEventFarming = ({ cfg } = {}) => {
  ...
  const loop = async () => {
    ...
    try {
      ...
      if (isInJoinableEventContext() && !shouldRangerBypassJoinableEvent())
        return;
      ...
      if (
        eventCombatEnabled &&
        isInCombatJoinableEvent() &&
        !shouldRangerBypassJoinableEvent()
      ) {
        return;
      }
      ...
      if (character.name === leader) {
        ...
        // Seasonal world event flows should not be held back by the
        // highest-luck ranger rare-mob gating logic.
        // These are explicit seasonal world event targets, not the underlying
        // seasonal joinable event state flags like egghunt/halloween.
        const seasonalWorldEventNames = new Set([
          "wabbit",
          "snowman",
          "mrpumpkin",
          "mrgreen",
        ...
        const isSeasonalWorldEvent = Boolean(
          worldEvent &&
          seasonalWorldEventNames.has(
            String(worldEvent.name || "").toLowerCase(),
          ),
        );
        const isWabbitWorldEvent =
          String(worldEvent?.name || "").toLowerCase() === "wabbit";
        ...
        shouldHoldCrabForHighestLuckRanger =
          ...
          !hasNearbyRareMob &&
          !isSeasonalWorldEvent;

        allowHighestLuckRangerInWorldEvent =
          isCurrentHighestLuckRanger &&
          (hasNearbyRareMob || isSeasonalWorldEvent || isLowEasterLuck);
