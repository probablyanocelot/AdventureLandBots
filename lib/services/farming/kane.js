...
const getCitizen0AuraMs = () => {
  const statuses = character?.s;
  if (!statuses || typeof statuses !== "object") return 0;
  const aura = statuses?.citizen0aura;
  if (!aura || typeof aura !== "object") return 0;
  const ms = Number(aura?.ms);
...
const recordKaneMovement = (st, kane, cfg) => {
  ...
  const threshold = Number(
    citizenConfig.movementThresholdPx ?? CITIZEN0_DEFAULTS.movementThresholdPx,
  );
...
const shouldUseCitizen0Consumable = (st, cfg) => {
  ...
  const neededDelay = Number(
    citizenConfig.auraStopWaitMs ?? CITIZEN0_DEFAULTS.auraStopWaitMs,
  );
...
const handleKaneCrabRoutine = async ({
  ...
  const leashRadiusPx = Number(
    citizenConfig.leashRadiusPx ?? citizenConfig.proximityDistancePx ?? 18,
  );
  const distanceToKane = distanceBetween(character, kane);

  recordKaneMovement(st, kane, cfg);

  const auraSaved = getCitizen0AuraMs() > 0;
  if (auraSaved) {
  ...
  if (shouldUse) {
    if (distanceToKane > leashRadiusPx) {
      moveToPoint({
        target: { map: kane.map, x: kane.x, y: kane.y },
        st,
        mover,
        cfg,
        proximityOverridePx: leashRadiusPx,
      });
      return true;
    }
