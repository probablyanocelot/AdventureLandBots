const now = () => Date.now();
const CONFETTI_ALWAYS_INTERVAL_MS = 19000;
const { getFarmingConfig } = await require("./runtime_helpers.js");
...
const CITIZEN0_DEFAULTS = Object.freeze({
  ...
  confettiCooldownMs: CONFETTI_ALWAYS_INTERVAL_MS,
  firecrackersCooldownMs: 200000,
  confettiThrillMs: CONFETTI_ALWAYS_INTERVAL_MS,
  firecrackersThrillMs: 200000,
  proximityDistancePx: 90,
});

const getCitizen0Config = (cfg) => {
  const raw = getFarmingConfig(cfg).citizen0;
  if (raw === false) {
...
const stopSmartMoveIfRunning = () => {
  try {
    if (!smart?.moving) return;
    if (typeof stopRoutine === "function") {
      stopRoutine("smart");
      return;
    }
...
const getCitizen0ReadyItem = (st, cfg) => {
  ...
  const confettiCooldownMs = CONFETTI_ALWAYS_INTERVAL_MS;
...
const isCitizen0MovingTowardCharacter = (kane) => {
  const goingX = Number(kane?.going_x);
  const goingY = Number(kane?.going_y);
  if (!Number.isFinite(goingX) || !Number.isFinite(goingY)) return false;

  const sameMap =
    kane?.map && character?.map
      ? String(kane.map) === String(character.map)
      : true;
  if (!sameMap) return false;

  const currentDistance = distanceBetween(kane, character);
  const projectedDistance = distanceBetween(
    { x: goingX, y: goingY },
    character,
  );
  return projectedDistance + 1 < currentDistance;
};
...
const getFormCrabSpot = (cfg) => {
  const defaultSpot = getFarmingConfig(cfg).huntSpotsByTarget?.crab?.[0];
  if (
    defaultSpot &&
    defaultSpot.map &&
...
const handleKaneCrabRoutine = async ({
  ...
  const farmAttackUnblockDistancePx = Number(
    citizenConfig.farmAttackUnblockDistancePx ?? pullDistancePx,
  );
  ...
  const lureStillActive =
    Number.isFinite(Number(st.kaneLureActiveUntil)) &&
    nowMs < Number(st.kaneLureActiveUntil);
  const kaneMovingTowardCharacter = isCitizen0MovingTowardCharacter(kane);
  const shouldSuppressChase = lureStillActive && kaneMovingTowardCharacter;
  const shouldChaseForLeash =
    (drift.notClose || drift.walkingAway) &&
    !shouldSuppressChase &&
    (hasConsumable || lureStillActive);

  if (!hasConsumable && !lureStillActive && !st.kaneActionState) {
    return false;
  }
  ...
  const shouldRelure =
    Boolean(readyItem) &&
    (readyItem?.name === "confetti" || shouldChaseForLeash);
  ...
  if (st.kaneActionState === "return" && nowMs <= st.kaneReturnUntil) {
    const nearFarmForCombat =
      distanceBetween(character, farmPoint) <= farmAttackUnblockDistancePx;
    if (nearFarmForCombat) {
      stopSmartMoveIfRunning();
