...
const recordKaneMovement = (st, kane, cfg) => {
  ...
  const dx = Math.abs(Number(kane.x || 0) - Number(st.kaneInitialPosition.x || 0));
  const dy = Math.abs(Number(kane.y || 0) - Number(st.kaneInitialPosition.y || 0));

  const citizenConfig = getCitizen0Config(cfg);
  const threshold = Number(citizenConfig.movementThresholdPx || 150);
...
const shouldUseCitizen0Consumable = (st, cfg) => {
  ...
  const confettiReady =
    ...
    nowMs - st.lastKaneConfettiAt >= Number(citizenConfig.confettiCooldownMs || 20000);

  const firecrackersReady =
    !Number.isFinite(st.lastKaneFirecrackersAt) ||
    nowMs - st.lastKaneFirecrackersAt >= Number(citizenConfig.firecrackersCooldownMs || 200000);
...
const useCitizen0Consumable = (st, cfg) => {
  ...
  const firecrackersCooldownMs = Number(citizenConfig.firecrackersCooldownMs || 200000);
...
const moveToPoint = ({ target, st, mover, cfg }) => {
  ...
  const citizenConfig = getCitizen0Config(cfg);
  const proximity = Number(citizenConfig.proximityDistancePx || st.kaneApproachDistancePx || 70);
...
const handleKaneCrabRoutine = async ({ cfg, st, mover, effectiveIsTiny }) => {
  ...
  if (shouldUse) {
    ...
    if (item) {
      ...
      st.kaneApproachUntil = nowMs + Number(citizenConfig.approachDurationMs || 9000);
      st.kaneReturnUntil = st.kaneApproachUntil + Number(citizenConfig.returnDurationMs || 12000);
