...
const { runFarmingTarget, runFarmingAssignment } =
  await require("./farming_runner.js");
...
const installFarming = ({ cfg } = {}) => {
  ...
  const loop = async () => {
    ...
    try {
      ...
      if (localForcedHunt) {
        const directResult = await runFarmingTarget({
          cfg,
          st,
          target: manualFarmMobSelf,
          isSelf: true,
          logOnce,
          getSafeSpot,
          moveToSafeSpot,
          ...
          runWorldEvent,
          runCrab,
          runFarmingHunt: runMonsterhunt,
          getMonsterhuntTarget,
          runMageSupport,
          runPriestSupport,
          handleKaneCrabRoutine,
        });
        if (directResult?.abortLoop) return;
        return;
      }

      let assignment = st.lastAssignment;
      const isAssignedToFarm =
        Boolean(assignment?.monsterhunt?.includes(character.name)) ||
        Boolean(assignment?.crab?.includes(character.name));
      const runManualGroupFallback = manualFarmMob && !isAssignedToFarm;

      if (runManualGroupFallback) {
        const directResult = await runFarmingTarget({
          cfg,
          st,
          target: manualFarmMob,
          isSelf: false,
          logOnce,
          getSafeSpot,
          moveToSafeSpot,
      ...
      const executionResult = await runFarmingAssignment({
        cfg,
        st,
        assignment,
        localForcedHunt,
