...
const installFarming = ({ cfg } = {}) => {
  ...
  const buildDirectFarmingAssignment = ({ target, modePrefix } = {}) => {
    return {
  ...
  const runDirectFarming = async ({ target, isSelf } = {}) => {
    if (!target || typeof target !== "string") return { abortLoop: false };
    const assignment = buildDirectFarmingAssignment({
      target,
  ...
  const loop = async () => {
    ...
    try {
      ...
      if (localForcedHunt) {
        const directResult = await runDirectFarming({
      ...
      const groupFallbackTarget = manualFarmMob && !st.lastAssignment;
      if (groupFallbackTarget) {
        const directResult = await runDirectFarming({
