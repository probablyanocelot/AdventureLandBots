  ...
  return (m) => {
    try {
      ...
      if (data.cmd === "farm:roles") {
        ...
        // clear blocked ranger hold when world event is porcupine
        if (
          data.assignment?.worldEvent?.name &&
          String(data.assignment.worldEvent.name).toLowerCase() === "porcupine"
        ) {
          st.blockedRangers?.clear?.();
        }
        ...
        const isRanger =
          String(character?.ctype || "").toLowerCase() === "ranger";
        const isOnlyRanger = Number(st.rangerCount || 0) <= 1;
        const wantsMonsterhunt =
          Array.isArray(data.assignment?.monsterhunt) &&
          data.assignment.monsterhunt.includes(characterName);

        if (isRanger && isOnlyRanger && wantsMonsterhunt) {
          try {
            sendCm?.(m.name, {
              cmd: "farm:monsterhunt_decline",
              from: characterName,
              target: data.assignment?.worldEvent?.name || null,
            });
          } catch {
            // ignore
          }
      ...
      if (data.cmd === "farm:monsterhunt_decline") {
        if (typeof m.name === "string" && m.name) {
          st.blockedRangers = st.blockedRangers || new Set();
          st.blockedRangers.add(m.name);
        }
        return;
      }
