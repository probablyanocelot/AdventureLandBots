...
const spreadFromPartyIfStacked = ({ cfg, huntGroupNames = [] } = {}) => {
  try {
    ...
    const moveCooldownMs = Math.max(
      120,
      normalizeNumber(cfg?.farming?.stackedSpreadCooldownMs, 220),
    );
    if (
    ...
    const clusterRadius = Math.max(
      12,
      normalizeNumber(cfg?.farming?.stackedClusterRadius, 35),
    );
    ...
    const spreadStep = Math.max(
      10,
      normalizeNumber(cfg?.farming?.stackedSpreadStep, 20),
    );
