...
const installUpkeep = ({ cfg } = {}) => {
  ...
  const tryUpkeep = (kind) => {
    ...
    if (kind === "hp") {
      ...
      // For small HP deficits, use regen first.
      ...
      // Otherwise prefer potions (hpot1 first, then hpot0).
    ...
    if (kind === "mp") {
      ...
      // While gathering, skip only minor MP top-offs to avoid excessive interruption.
      if (!canInterruptGatheringForMp) return false;

      // For small MP deficits, use regen first.
      if (shouldPreferRegen) {
      ...
      // Otherwise prefer potions (mpot1 first, then mpot0).
      ...
      // No MP potions available: fall back to regen if possible.
      if (!is_on_cooldown("regen_mp")) {
        use_skill("regen_mp");
        return true;
      }
