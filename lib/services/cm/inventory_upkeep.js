...
// TODO: consider refactoring into separate MP/HP upkeep and economy upkeep routines. This should be focused on acquiring items needed for general farming, etc.

const { getConfig } = await require("../../config/index.js");
const { warn } = await require("../../al_debug_log.js");
const { now } = await require("../helpers/time/index.js");
const { getLoc } = await require("../helpers/movement/move_manager.js");
const { runPriestSupport } = await require("../combat/index.js");
const { createItemUpgradeService } = await require("./item_upgrade.js");
const { sellList, noExchange } = await require("../data/index.js");

const COMPUTER_AUTO_SELL_NAMES = sellList;
const DEFAULT_COMPOUNDABLE_EXCHANGE_EXCLUDES = Object.keys(noExchange);
