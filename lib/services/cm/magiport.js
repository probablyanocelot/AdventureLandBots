// Magiport domain routine.
// Purpose: safely port targets to mage location with CM prepare/ack handshake.
// Inputs: target list, optional task metadata, config, force/ttl options.
// Side effects: sends CM preflight messages and casts `use_skill("magiport", target)`.
// Safety: skips joinable-event and nearby targets unless `force` is set.

const { getConfig } = await require("../../config/index.js");
const { sleepMs, now } = await require("../shared/index.js");
const { isNearby } = await require("../state/flags.js");
const { info, warn } = await require("../../al_debug_log.js");
const { waitForCm } = await require("../runtime-listeners/index.js");
const { isJoinableEvent } = await require("../events/server_event_catalog.js");
...
const prepareTarget = async ({ targetName, mageName, taskId, ttlMs }) => {
  ...
  // Clear any stale expectation first so a previous mage/request can't be consumed.
  try {
...
async function magiportTargets({
  ...
  const cfg = getConfig();
  const mageName = character?.name || cfg.mageName;
