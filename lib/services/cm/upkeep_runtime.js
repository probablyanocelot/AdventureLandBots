...
const { hasConsumableInSlots } = await require("../inventory/consumables.js");
...
const GATHERING_MP_RECOVERY_RATIO = 0.2;
...
const installUpkeep = ({ cfg } = {}) => {
  ...
  const tryEnsureElixir = (elixirName) => {
    const ELIXIR_NAME = elixirName;

    if (hasConsumableInSlots(character, ELIXIR_NAME)) {
      return false; // already in elixir/buff slot
    }
    ...
    const itemSlot = locate_item(ELIXIR_NAME);
    ...
    try {
      ...
      warn(`Failed to auto-use ${ELIXIR_NAME}`, e);
      return false;
    }
  };
  ...
  const tryUpkeep = (kind) => {
    ...
    if (kind === "hp") {
      ...
      // If both are on cooldown or unavailable
      return false;
    }
    ...
    if (kind === "mp") {
      ...
      const mpRatio =
        Number(character?.max_mp || 0) > 0
          ...
          : 1;
      const canInterruptGatheringForMp = !isActivelyGathering;
  ...
  const loop = () => {
    ...
    try {
      ...
      tryEnsureElixir("elixirluck");

      const skipPumpkinSpiceForRogue = Boolean(
        cfg?.merchantAssist?.skipPumpkinSpiceWhenRogue,
      );
      if (character.ctype === "rogue" && skipPumpkinSpiceForRogue) {
        tryEnsureElixir("pumpkinspice");
      }
