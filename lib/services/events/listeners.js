...
const { spreadFromPartyIfStacked } = await require("../helpers/combat/index.js");
...
const stopGlobalRuntimeListeners = () => {
  ...
  try {
    if (runtimeGlobalsState.offJackoIncoming) runtimeGlobalsState.offJackoIncoming();
  } catch {
    // ignore
  }
  ...
  try {
    if (runtimeGlobalsState.offJackoMobbed) runtimeGlobalsState.offJackoMobbed();
  } catch {
    // ignore
  }
  ...
  try {
    ...
    runtimeGlobalsState.offStacked = onCharacter("stacked", () => {
      try {
        ...
        const cfg = getConfig?.() || {};
        if (cfg?.noEventFarming?.enableStackedSpreadListener === false) return;
    ...
    runtimeGlobalsState.offScare = onCharacter("scare", () => {
      try {
        ...
        const cfg = getConfig?.() || {};
        if (cfg?.noEventFarming?.enableScareListener === false) return;
    ...
    const maybeDoJackoScare = async (actor) => {
      ...
      if (typeof is_on_cooldown === "function" && is_on_cooldown("scare")) return;
      if (typeof use_skill !== "function" || typeof equip !== "function") return;
      ...
      try {
        if (!hasJackoInOrb) {
          if (jacko.type === "inventory") {
            ...
            const jackoInvIndex = (character.items || []).findIndex((it) => it?.name === "jacko");
            if (jackoInvIndex >= 0) {
      ...
      if (!hasJackoInOrb && originalOrbName) {
        const fallbackIndex = (character.items || []).findIndex(
          (it) => it?.name === originalOrbName && it?.level === originalOrbLevel,
        );
        if (fallbackIndex >= 0) {
    ...
    runtimeGlobalsState.offJackoIncoming = onCharacter("incoming", handleIncomingJackoScare);
