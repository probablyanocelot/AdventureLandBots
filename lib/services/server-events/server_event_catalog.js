...
const SEASONAL_BLOCKED_EVENTS = new Set(["icegolem", "crabxx", "abtesting"]);
...
const isBlockedBySeason = (name) => {
  if (!name) return false;
  if (!SEASONAL_BLOCKED_EVENTS.has(name)) return false;
...
const getActiveWorldEvents = () => {
  try {
    ...
    for (const [name, val] of Object.entries(active)) {
      ...
      if (typeof val.live === "boolean" && !val.live) continue;
      if (!val.map) continue;

      const hasCoords = Number.isFinite(val.x) && Number.isFinite(val.y);
      // Some event monsters (e.g. wabbit) only broadcast the map; we need to still track them.
      if (!hasCoords && !val.map) continue;

      if (isBlockedBySeason(name)) continue;
