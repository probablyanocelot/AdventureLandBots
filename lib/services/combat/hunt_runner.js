...
const runMonsterhunt = ({
  ...
  try {
    ...
    const mvampireCheckIntervalMs = Math.max(
      0,
      Number(nf.mvampireCheckIntervalMs || 5 * 60 * 1000),
    );
    const lastMvampireCheckAt = Number(cfg?._lastMvampireCheckAt || 0);
    const hasMvampireBuff = Boolean(character?.s?.mlifesteal || false);
    ...
    if (
      ...
      if (waypoints.length >= 2) {
        ...
        if (cfg?.debug?.combat) {
          dbg(
            cfg,
            "hunt_mvampire_sweep_start",
            "starting mvampire spawn sweep",
            {
              targets: waypoints,
            },
          );
    ...
    if (st?.mvampireSweep?.active && !hasGoldenbat) {
      ...
      const nextWaypoint = sweep.waypoints[sweep.currentStep];
      if (nextWaypoint) {
        target = "mvampire";
      }
    }
    ...
    if (st?.mvampireSweep?.active) {
      ...
      if (waypoint && waypoint.map === character?.map) {
        huntDest = waypoint;
      } else if (waypoint) {
        huntDest = waypoint;
      }
    ...
    const teammateAtDestination =
      nearDestinationAnchor ||
      (character?.ctype === "ranger" && !isPorcupineTarget)
        ? null
