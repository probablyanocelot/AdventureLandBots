...
const runMonsterhunt = ({
  ...
  try {
    ...
    if (hardByDefinition && priestRequiredForWarriorPull && !priestPresent) {
      if (tracker) {
        tracker.add({
    ...
    const priestBackupRequiredForEngage =
      character?.ctype === "warrior" ? requirePriestForWarriorHardPull : true;

    const warriorHardPullBlocked =
      isHardTarget && priestRequiredForWarriorPull && !priestPresent;
