...
const pickHuntDestination = (target, cfg) => {
  try {
    ...
    // If config spot, try to find closest boundary corner if any spawn matches map and has boundary
    if (
      best._cfgPreferred &&
      typeof best._cfgX === "number" &&
      typeof best._cfgY === "number"
    ) {
      // Find all spawns for this map/target
      let bestCorner = null;
      let bestDist = Infinity;
      for (const [mapName, mapDef] of Object.entries(G.maps)) {
        if (mapName !== best.map) continue;
        const monsters = Array.isArray(mapDef?.monsters) ? mapDef.monsters : [];
        for (const spawn of monsters) {
          if (!spawn || spawn.type !== target) continue;
          const corners = getBoundaryCorners(spawn.boundary);
          for (const c of corners) {
            const d = Math.hypot(c.x - best._cfgX, c.y - best._cfgY);
            if (d < bestDist) {
              bestDist = d;
              bestCorner = c;
            }
      ...
      if (bestCorner) {
        console.log("pickHuntDestination-bestCorner", { target, cfgSpot: best, chosen: result });
        return {
          map: best.map,
          x: bestCorner.x,
          y: bestCorner.y,
          ...
          corner: { x: bestCorner.x, y: bestCorner.y },
        };
      }
      // fallback to config x,y
      console.log("pickHuntDestination-fallback", { target, cfgSpot: best, chosen: result });
      return {
        map: best.map,
        x: best._cfgX,
        y: best._cfgY,
    ...
    const chosen =
      isStrong && corners.length
        ...
        console.log("pickHuntDestination-end", { target, cfgSpot: best, chosen: result });
    return {
      map: best.map,
      x: chosen.x,
      y: chosen.y,
...
const runMonsterhunt = ({
  ...
  try {
    ...
    if (spreadFromPartyIfStacked({ cfg, huntGroupNames })) return;
    ...
    if (isHardTarget && (!groupArrived || !priestBackupReady)) {
      dbg(
        cfg,
