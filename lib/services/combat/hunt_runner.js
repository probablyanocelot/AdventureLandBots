...
const pickHuntDestination = (target, cfg) => {
  try {
    ...
    let useCorner = false;
    // Determine if monster is strong (high hp/attack/long fight)
    if (isTargetDifficult(target, cfg)) {
      useCorner = true;
    }
    const chosen =
      useCorner && corners.length
...
const runCrab = ({ cfg, mover } = {}) => {
  ...
  // Fallback disabled: do nothing if no crab is found
