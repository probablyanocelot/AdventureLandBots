...
const tryAttack = (target, cfg, st) => {
  ...
  try {
    if (character?.ctype === "rogue") {
      ...
      const hpRatio = Number(character?.hp || 0) / maxHp;
      const isLowHp = hpRatio < 0.33;
      const isRecovered = hpRatio >= 0.75;

      if (isLowHp && !isRecovered && target?.target === character?.name) {
        // Hold off on attacking until target switches or rogue heals.
        return;
      }
    }
  } catch {
  ...
  const cadenceMs = getAttackCadenceMs({ cfg });
  const cadenceReady = shouldAttackByCadence({
    lastAttackAt: st?.lastAttackAt,
    cadenceMs,
  });
  ...
  if (shouldAttemptAttack && cadenceReady) {
    try {
      attack(target);
      if (st) st.lastAttackAt = Date.now();
      dbg(
        cfg,
...
const installEventCombat = ({ cfg } = {}) => {
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (st.stopped) return;
      st.timer = setTimeout(tick, Math.min(300, getAttackCadenceMs({ cfg })));
    }
  };
