...
const isDeniedHuntSpot = (target, spot, cfg) => {
  try {
    ...
    const allowByTarget = cfg?.farming?.huntMapAllowByTarget;
    ...
    const denyByTarget = cfg?.farming?.huntMapDenyByTarget;
...
const isPreferredHuntSpot = (target, spot, cfg) => {
  try {
    if (!target || !spot?.map) return false;
    const preferByTarget = cfg?.farming?.huntMapPreferByTarget;
...
const pickHuntDestination = (target, cfg) => {
  try {
    ...
    const cfgSpots = cfg?.farming?.huntSpotsByTarget?.[target];
