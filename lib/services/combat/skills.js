...
const pick5ShotTargets = (primaryTarget) => {
  const monsters = getInRangeMonsters({ max: 30 });
...
const shouldUseOccasional5Shot = (target) => {
  try {
    ...
    if (!hasSkillDef("5shot") || !canUseSkill("5shot")) return false;
...
const getHighPhysicalAttackThreshold = () => {
  try {
    const fromRuntime = Number(
      globalThis?.AL_BOTS_CONFIG?.noEventFarming?.highAttack,
    );
...
const useFarmerSkills = (target, { disableMultiTarget = false } = {}) => {
  ...
  switch (character?.ctype) {
    ...
    case "paladin": {
      ...
      if (hasSkillDef("purify") && Number(target?.hp || 2001) <= 2000) {
