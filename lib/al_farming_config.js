...
const installNoEventFarming = ({ cfg } = {}) => {
  ...
  const loop = async () => {
    ...
    try {
      ...
      const assignment = st.lastAssignment;
      const assignmentAge = now() - st.lastAssignmentAt;
      const ttlMs = Math.max(8000, Number(nf.assignmentTtlMs || 12000));
      const continueWhileLeaderBusyMs = Math.max(
        ttlMs,
        Number(nf.continueWhileLeaderBusyMs || 45000),
      );

      if (!assignment) {
        moveToSafeSpot({ key: "no_assignment_safe", priority: 1 });
        return;
      }

      if (assignmentAge > continueWhileLeaderBusyMs) {
        moveToSafeSpot({ key: "stale_assignment_safe", priority: 1 });
        return;
      }
      ...
      // If leader is between turn-in/new hunt assignment, followers can keep farming
      // for a while using the latest assignment.
      if (assignmentAge > ttlMs && !isTiny && !isHunt) {
        moveToSafeSpot({ key: "idle_safe", priority: 1 });
      }
