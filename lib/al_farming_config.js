...
const listOfflineFarmerNamesByType = ({
  ...
  const active = new Set((Array.isArray(activeRoster) ? activeRoster : []).filter(Boolean));
...
const installNoEventFarming = ({ cfg } = {}) => {
  ...
  const loop = async () => {
    ...
    try {
      ...
      const liveTarget = manualFarmMob || getMonsterhuntTarget();
      ...
      if (character.name === leader && !manualFarmMob && needsMonsterhuntTurnIn()) {
        const nowMs = now();
        const priorTarget =
      ...
      if (character.name === leader) {
        ...
        if (isPorcupineTarget(target)) {
          ...
          const rangerOnline = roster.some(
            (name) => meta.get(name)?.ctype === "ranger",
          );
          ...
          const canTrySwap =
            onlineMelee.length > 0 && nowMs - st.lastPorcupineSwapAt >= porcupineSwapCooldownMs;
          ...
          if (canTrySwap) {
            ...
            const codeSlotOrName = cfg?.mageSwap?.codeSlotOrName;
            if (
              codeSlotOrName !== undefined &&
              codeSlotOrName !== null &&
              codeSlotOrName !== "" &&
        ...
        const rallySource = canReuseRecentHuntRally
          ? recentHuntStats.map((it) => it.stat).find(
              (s) =>
                s &&
                typeof s.x === "number" &&
                typeof s.y === "number" &&
      ...
      const shouldSkipMeleePorcupine =
        isHunt && isPorcupineTarget(sharedTarget) && isMeleeCtype(character?.ctype);
      ...
      if (worldEvent && isHunt) {
        ...
        if (shouldSkipMeleePorcupine) {
          moveToSafeSpot({ key: "porcupine_melee_hold", priority: 2 });
        }
        if (isHunt && !shouldSkipMeleePorcupine)
