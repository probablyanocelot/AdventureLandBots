...
const requestMageMagiport = ({ mageName, targetName, taskId } = {}) => {
  try {
...
const isCharacterOnline = (name) => {
  try {
    ...
    return Array.isArray(active) && active.includes(name);
  } catch {
    return false;
  }
};
...
const stopCharacterSafe = (name) => {
  try {
    if (!name || name === character?.name) return false;
    stop_character(name);
    return true;
  } catch {
    return false;
...
const ensureChainMageRunning = async ({
  ...
  try {
    if (!mageName) return { ok: false, reason: "missing-mage" };
    if (isCharacterOnline(mageName)) {
      return { ok: true, alreadyRunning: true, mageName };
    }
...
const installNoEventFarming = ({ cfg } = {}) => {
  ...
  const loop = async () => {
    ...
    try {
      ...
      if (
        chainCfg.enabled &&
        chainCfg.warriorName &&
        roster.includes(chainCfg.warriorName)
      ) {
        leader = chainCfg.warriorName;
        st.leader = leader;
      }
      ...
      if (character.name === leader && st.pendingAggroChain) {
        ...
        if (age > timeoutMs) {
          ...
          if (!pending.sameTargetOnly || sameTarget) {
            const cooldownMs = Math.max(
              1500,
              Number(chainCfg.requestCooldownMs || 6000),
            );
            if (now() - st.lastAggroChainFinalizeAt >= cooldownMs) {
              ...
              const selectedHuntMage = readyHuntMage.ok
                ? pending.huntMageName
                : cfg.mageName || pending.huntMageName;
              const ok = requestMageMagiport({
                mageName: selectedHuntMage,
                targetName: pending.warriorName,
                taskId: `huntchain:hunt:${now()}`,
              });
              st.lastAggroChainFinalizeAt = now();

              if (
                ok &&
                chainCfg.stopNpcMageAfterChainPort !== false &&
                pending.npcMageName &&
                pending.npcMageName !== selectedHuntMage &&
      ...
      if (character.name === leader && needsMonsterhuntTurnIn()) {
        const nowMs = now();
        const priorTarget =
          getMonsterhuntTarget() || st.lastAssignment?.huntTarget || null;
        ...
        const chainEnabled =
          ...
          Boolean(priorTarget) &&
          character.name === (chainCfg.warriorName || character.name);
        ...
        if (
          ...
          if (!requirePriestAggro || priestHolding) {
            const npcMageName = chainCfg.npcMageName || cfg.mageName || null;
            const huntMageName =
              chainCfg.huntMageName ||
              chainCfg.npcMageName ||
              cfg.mageName ||
              null;
            const warriorName = chainCfg.warriorName || character.name;
            ...
            const selectedNpcMage = readyNpcMage.ok
              ? npcMageName
              : cfg.mageName || npcMageName;

            const npcPortOk = requestMageMagiport({
              mageName: selectedNpcMage,
              targetName: warriorName,
              taskId: `huntchain:npc:${nowMs}`,
            });
      ...
      if (character.name === leader) {
        ...
        if (chainCfg.enabled) {
          const warriorName = chainCfg.warriorName || null;
          const priestName = chainCfg.priestName || null;
          const huntMageName = chainCfg.huntMageName || null;
          const npcMageName = chainCfg.npcMageName || null;
