...
async function main() {
  ...
  const startTelemetryIfEnabled = async () => {
    try {
      if (cfg?.telemetry?.enabled === false) return;
      const { installTelemetry } = await require("./telemetry/client.js");
      installTelemetry({ cfg });
    } catch (e) {
  ...
  await startTelemetryIfEnabled();

  switch (character.ctype) {
    case "merchant": {
      // Current implementation exports `Merchant` (extends BotCharacter).
      const { Merchant } = await require("./class_merchant.js");
    ...
    case "mage": {
      const { Mage } = await require("./class_mage.js");
      MageCtor = Mage;
      bot = new MageCtor();
    ...
    case "warrior": {
      const { Warrior } = await require("./class_warrior.js");
      WarriorCtor = Warrior;
      bot = new WarriorCtor();
    ...
    case "ranger": {
      const { Ranger } = await require("./class_ranger.js");
      RangerCtor = Ranger;
      bot = new RangerCtor();
    ...
    case "priest": {
      const { Priest } = await require("./class_priest.js");
      PriestCtor = Priest;
      bot = new PriestCtor();
    ...
    case "rogue": {
      const { Rogue } = await require("./class_rogue.js");
      RogueCtor = Rogue;
      bot = new RogueCtor();
    ...
    case "paladin": {
      const { Paladin } = await require("./class_paladin.js");
      PaladinCtor = Paladin;
      bot = new PaladinCtor();
