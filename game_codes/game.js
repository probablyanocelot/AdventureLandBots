    ...
    if (reload_state == "start") {
      ((reload_state = "schedule"),
        (reload_timer = future_s(
          (window.rc_delay || 0) + 3 + parseInt(Math.random() * 2),
        )));
      add_log(
...
function disconnect() {
  ...
  if (
    character &&
    (auto_reload == "on" ||
      (auto_reload == "auto" && (character.stand || code_run || 1)))
    ...
    if (0) {
      ...
      $("body")
        ...
        .each(function () {
          if (
            this.tagName != "CANVAS" &&
            this.id != "bottomrightcorner" &&
            this.id != "bottomleftcorner2"
          )
            $(this).remove();
          else if (
            this.id == "bottomrightcorner" ||
...
function position_map() {
  ...
  if (change && dtile_size && window.dtile) {
    ...
    dtile.x =
      ceil(dtile.x / (dtile_size / 1.0)) * (dtile_size / 1.0) -
      dtile_size / 1.0;
    dtile.y =
      ceil(dtile.y / (dtile_size / 1.0)) * (dtile_size / 1.0) -
      dtile_size / 1.0;
  }
  if (change && window.wtile) {
    ...
    wtile.x =
      ceil(wtile.x / (textures[wtile_name][0].width / 1.0)) *
        (textures[wtile_name][0].width / 1.0) -
      textures[wtile_name][0].width / 1.0;
    wtile.y =
      ceil(wtile.y / (textures[wtile_name][0].height / 1.0)) *
        (textures[wtile_name][0].height / 1.0) -
      textures[wtile_name][0].height / 1.0;
  }
  if (character) {
    if (manual_centering)
      ((character.x = c_round(width / 2.0 + ch_disp_x)),
        (character.y = c_round(height / 2.0 + ch_disp_y)));
    else
...
function reset_topleft() {
  ...
  if (ctarget && ctarget.type == "monster" && last_target_cid != ctarget.cid) {
    ...
    ctarget &&
    ctarget.type == "character" &&
    last_target_cid != ctarget.cid
  ) {
  ...
  if (current.move_num != current.engaged_move) {
    ...
    if (dist > 120) // previously 40 [07/10/16]
    {
      ...
      if (log_flags.xy_corrections)
        console.log(
          "manual x,y correction for: " + (current.name || current.id),
        );
    }
    speedm =
      simple_distance(
        { x: current.real_x, y: current.real_y },
        { x: monster.going_x, y: monster.going_y },
      ) /
      (simple_distance(monster, { x: monster.going_x, y: monster.going_y }) +
        EPS);
  ...
  for (var id in future_entities.monsters) {
    ...
    if (!current) {
      ...
      try {
        ...
        console.log(
          "EMAIL HELLO@ADVENTURE.LAND WITH THIS: " + JSON.stringify(monster),
        );
...
function adopt_soft_properties(element, data) {
  ...
  if (element.type == "monster" && G.monsters[element.mtype]) {
    ...
    mapping.forEach(function (attr) {
      ...
      if (
        def[attr[1]] !== undefined &&
        (data[attr[0]] === undefined || element[attr[0]] === undefined)
      ) {
...
function update_tutorial_ui() {
  ...
  $("#tutorialslider").css(
    "width",
    ((X.tutorial.step + 1) * 100) / G.docs.tutorial.length + "%",
  );
...
function update_overlays() {
  ...
  if (character) {
    ...
    if (!cached("hptop", character.hp, character.max_hp)) {
      ...
      $("#hpslider").css(
        "width",
        (character.hp * 100) / character.max_hp + "%",
      );
    }
    if (!cached("mptop", character.mp, character.max_mp)) {
      ...
      $("#mpslider").css(
        "width",
        (character.mp * 100) / character.max_mp + "%",
      );
    }
    ...
    if (!cached("xptop", character.level + "|" + xp)) {
      ...
      $("#xpslider").css(
        "width",
        (character.xp * 100) / character.max_xp + "%",
      );
    }
    ...
    if (inventory && !cached("igold", character.gold))
      $(".goldnum").html(
        to_pretty_num(
          character.gold +
            (new Date().getDate() == 101 && new Date().getMonth() == 3
              ? 1014201800
              : 0),
        ),
      );
...
function showhide_quirks_logic() {
  ...
  (G.maps[character.map].quirks || []).forEach(function (q) {
    if (
      q[4] == "info" &&
      point_distance(character.real_x, character.real_y, q[0], q[1]) < 200
    ) {
      quirks[q[5]] = true;
    }
  });
  (G.maps[character.map].zones || []).forEach(function (zone) {
    [
      ...
      if (
        is_point_inside(
          [character.real_x + m[0], character.real_y + m[1]],
          zone.polygon,
        )
      ) {
  ...
  if (PIXI.DisplayList && !no_graphics) {
    ...
    var child_sort = function (sprite) {
      ...
      if ("real_y" in sprite.parent)
        sprite.zOrder =
          -sprite.parent.real_y + disp + (sprite.parent.y_disp || 0);
      else
        sprite.zOrder =
          -sprite.parent.position.y + disp + (sprite.parent.y_disp || 0);
    };
...
function init_demo() {
  ...
  G.maps[current_map].monsters.forEach(function (pack) {
    future_entities.monsters[pack.type] = {
      ...
      x:
        pack.boundary[0] +
        (pack.boundary[2] - pack.boundary[0]) * Math.random(),
      y:
        pack.boundary[1] +
        (pack.boundary[3] - pack.boundary[1]) * Math.random(),
...
function init_socket(args) {
  ...
  var query =
    (args.secret &&
      "desktop=" + ((!is_comm && 1) || "") + "&secret=" + args.secret) ||
    undefined;
  ...
  socket.emit = function (packet) {
    var is_transport = in_arr(arguments && arguments["0"], [
      "transport",
      "enter",
      "leave",
    ]);
    ...
    if (!(transporting && is_transport && ssince(transporting) < 8)) {
      ...
      resolve_deferred(arguments["0"], {
        success: false,
        in_progress: true,
        place: "emit_override_in_gamejs",
      });
    }
  };
  ...
  socket.on("start", function (data) {
    ...
    if (data.blessed_by) {
      add_chat(
        "",
        "This server has been blessed by " + data.blessed_by,
        "#8F70D8",
      );
    }
    ...
    try {
      ...
      if (data) {
        data = JSON.parse(data);
        the_code =
          ((!scode_slot || data["slot_" + real_id] == scode_slot) &&
            data["code_" + real_id]) ||
          code ||
          "";
  ...
  socket.on("game_event", function (data) {
    ...
    if (data.name == "pinkgoo") {
      add_chat(
        "",
        "The 'Love Goo' has respawned in " + G.maps[data.map].name + "!",
        "#EDB0E0",
      );
    }
    ...
    if (data.name == "wabbit") {
      add_chat(
        "",
        "Wabbit has respawned in " + G.maps[data.map].name + "!",
        "#78CFEF",
      );
    }
    if (data.name == "goldenbat") {
      add_chat(
        "",
        "The Golden Bat has spawned in " + G.maps[data.map].name + "!",
        "gold",
      );
    }
  ...
  socket.on("achievement_progress", function (data) {
    add_log(
      "AP[" +
        data.name +
        "]: " +
        to_pretty_num(data.count) +
        "/" +
        to_pretty_num(data.needed),
      "#6DCC9E",
    );
  });
    ...
    draw_trigger(function () {
      if (response == "elixir") {
        ...
        render_interaction({
          auto: true,
          skin: "lionsuit",
          message: "Set your home to: " + data.home,
        });
        character.home = data.home;
        ...
        render_interaction({
          ...
          skin: "lionsuit",
          message:
            "You can't change your home server for another " +
            to_pretty_float(data.hours) +
            " hours!",
        });
        ...
        if (data.gold)
          ui_log(
            "Opened an account for " + to_pretty_num(data.gold) + " gold",
            "gray",
          );
        else
          ui_log(
            "Opened an account for " + to_pretty_num(data.shells) + " shells",
            "gray",
          );
        ...
        ui_log(
          "To perform this action your account needs a game client authorization",
          "gray",
        );
        ...
        if ($("#topleftcornerdialog").length) {
          ...
          html +=
            ...
            "</div>";
          html +=
            "<div class='gamebutton' onclick='$(this).parent().remove()'>OK</div>";
          html += "</div>";
        ...
        if (response == "item_received") {
          ...
          add_chat(
            "",
            "Sent " + G.items[data.item].name + additional + " to " + data.name,
            "#6AB3FF",
          );
        ...
        add_log(
          "Received " + prefix + G.items[data.item.name].name + additional,
          "#3B9358",
        );
        ...
        add_chat(
          "",
          "Sent " + to_pretty_num(data.gold) + " gold to " + data.name,
          colors.gold,
        );
        ...
        add_chat(
          "",
          "Received " + to_pretty_num(data.gold) + " gold from " + data.name,
          colors.gold,
        );
      ...
      else if (response == "temporalsurge_none")
        ui_log("Temporal surge failed", "gray");
      else if (response == "temporalsurge")
        ui_log("Temporal surge hastened respawns!", "gray");
        ...
        if (data.level)
          ui_log(
            "Spent " + to_pretty_num(data.cost || 10000) + " gold",
            "gray",
          );
        ...
        ui_log(
          "It can be unlocked in " + parseInt(data.hours) + " hours",
          "gray",
        );
        ...
        render_interaction({
          auto: true,
          skin: "favore",
          message: "Thank you! The server has been blessed.",
        });
      } else if (response == "blessed_fail") {
        render_interaction({
          auto: true,
          skin: "favore",
          message: "Ooops! It failed.",
        });
      } else if (
        response == "monsterhunt_started" ||
        response == "monsterhunt_already"
      ) {
  ...
  socket.on("chat_log", function (data) {
    ...
    draw_trigger(function () {
      ...
      add_chat(
        ...
        data.message,
        data.color,
        (is_number(data.id) && data.id) || undefined,
      );
  ...
  socket.on("ui", function (data) {
    if (data.event)
      call_code_function(
        "trigger_event",
        (data.event === true && data.type) || data.event,
        data,
      );
    if (data.cevent && data.name == character.name)
      call_code_function(
        "trigger_character_event",
        (data.cevent === true && data.type) || data.cevent,
        data,
      );
    // show_json(data);
    draw_trigger(function () {
      if (in_arr(data.type, ["+$", "-$"])) {
        ...
        data.type == "frozen_resist" ||
        data.type == "deepfreezed_resist"
      ) {
        var target = get_entity(data.id);
        ...
        if (attacker)
          (points.push({ x: get_x(attacker), y: get_y(attacker) }),
            flurry(attacker));
  ...
  socket.on("upgrade", function (data) {
    ...
    draw_trigger(function () {
      ...
      map_npcs.forEach(function (npc) {
        ...
        if (
          npc.role == "newupgrade" &&
          (data.type == "upgrade" || data.type == "compound")
        ) {
  ...
  socket.on("hardcore_info", function (data) {
    ...
    if (data.achiever)
      add_chat(
        "mainframe",
        data.achiever + " ranked on the rewards list!",
        "#60B879",
      );
    render_server();
  });
  ...
  socket.on("action", function (data) {
    ...
    for (var name in data)
      if (!in_arr(name, ["attacker"])) event_data[name] = data[name];
    ...
    if (new_attacks) {
      if (
        G.projectiles[data.projectile] &&
        G.projectiles[data.projectile].animation
      )
    ...
    draw_trigger(function () {
      ...
      if (entity && data.goldsteal) {
        if (data.goldsteal > 0) {
          ...
          if (entity == character)
            add_log(
              "You lost " + to_pretty_num(data.goldsteal) + " gold",
              "#5D5246",
            );
        } else {
          d_text("+" + -data.goldsteal, entity, {
          ...
          if (entity == character)
            add_log(
              "Received " + to_pretty_num(-data.goldsteal) + " gold, huh",
              "#25B77D",
            );
        }
      }
      if (
        ...
        data.projectile &&
        !evade &&
        G.projectiles[data.projectile].hit_animation
      ) {
      ...
      if (
        ...
        data.projectile &&
        !evade &&
        G.projectiles[data.projectile].hit_text
      ) {
  ...
  socket.on("trade_history", function (data) {
    ...
    data.forEach(function (h) {
      ...
      if (h[0] == "buy") {
        ...
        html +=
          "<div>- Gave away " + prefix + "'" + item + "' to " + h[1] + "</div>";
      } else {
        html +=
          "<div>- Sold " +
          prefix +
      ...
      render_interaction(
        {
          ...
          skin: "brewingwitch",
          message:
            "My child, bring me the materials I seek and I shall unlock unimaginable horrors for you!",
        },
        "return_html",
      ),
    );
...
function map_click(event) {
  ...
  if (event && event.data && event.data.global) {
    ...
    if (
      ...
      return;
    if (
      (blink_pressed || mssince(last_blink_pressed) < 360) &&
...
function update_sprite(sprite) {
  ...
  if (sprite.stype == "full") // walk
  {
    ...
    if (
      (sprite.moving || aa || (sprite.fx && sprite.fx.aaa)) &&
      sprite.walking === null
    ) {
    ...
    if (
      ...
      ms_check(
        ...
        "walk",
        base_ms -
          ((sprite.speed + ((sprite.fx && sprite.fx.aaa && 500) || 0)) / 2 ||
            0),
      )
    )
    ...
    if (!aa && sprite.s && sprite.s.stunned) i = 1;
      ...
      sprite.last_stop &&
      mssince(sprite.last_stop) < ((sprite.s && sprite.s.dash && 5) || 180)
    )
      i = sequence[sprite.last_walking % sequence.length];

    if (
      (sprite.type == "character" || sprite.humanoid) &&
      (i === 0 || i === 2) &&
      original != i
    )
  ...
  if (
    (sprite.stype == "animation" || sprite.stype == "item") &&
    sprite.atype == "map"
  ) {
    ...
    sprite.crotation =
      Math.atan2(target_y - sprite.y, target_x - sprite.x) + Math.PI / 2;
    ...
    if (
      sprite.directional &&
      point_distance(target_x, target_y, sprite.x, sprite.y) > 50
    ) {
    ...
    if (sprite.to_fade)
      sprite.alpha -=
        (((sprite.to_fade !== true && sprite.to_fade) || 0.025) * ms) / 16.6;
    ...
    if (
      sprite.to_delete ||
      point_distance(target_x, target_y, sprite.x, sprite.y) <
        (sprite.limit || 16) ||
    ...
    sprite.rotation =
      Math.atan2(sprite.by - sprite.ay, sprite.bx - sprite.ax) + Math.PI / 2;
    if (sprite.alpha <= 0) {
      destroy_sprite(sprite, "children");
      delete map_animations[sprite.id];
      return;
  ...
  ) // map weather animation
  {
    ...
    if (ms >= sprite.interval) {
      sprite.texture =
        textures[sprite.skin][++sprite.last % textures[sprite.skin].length];
      sprite.last_update = new Date();
    }
  ...
  if (sprite.stype == "emote") {
    var steps =
      (sprite.aspeed == "slow" && 17) ||
      (sprite.aspeed == "slower" && 40) ||
      10; // not used [15/08/16]
    if (sprite.atype == "flow") {
      ...
      if (ms_check(sprite, "anim", steps * 16.5) && sprite.atype != "once")
        sprite.frame = (sprite.frame + 1) % 3; //sprite.updates%steps==1
      set_texture(sprite, sprite.frame);
    }
  }
      ...
      if (sprite.shuffling) {
        ...
        for (var j = sprite.locked; j < 4; j++) {
          ...
          if (
            sprite.lock_start &&
            j == sprite.locked &&
            mssince(sprite.lock_start) > (sprite.locked + 1) * 300
          ) {
            ...
            sprite.line = draw_line(
              0,
              0,
              min(34, sprite.seconds * 1.12),
              0,
              1,
              0x80e278,
            );
...
function add_monster(data) {
  ...
  if (def.size)
    ((sprite.height *= def.size),
      ...
      (sprite.mscale = def.size),
      (sprite.hpbar_wdisp = -5));
  if (def.orientation) sprite.orientation = def.orientation;
  sprite.interactive = true;
  sprite.buttonMode = true;
  set_base(sprite);
  sprite
    .on("mousedown", monster_click)
...
function start_filter(sprite, ftype, args) {
  ...
  if (ftype == "darkgray") {
    ...
    filter = new PIXI.filters.RadialBlurFilter(
...
function stop_filter(sprite, ftype) {
  ...
  sprite.filter_list.splice(
    sprite.filter_list.indexOf(sprite["filter_" + ftype]),
    1,
  );
...
function alpha_logic(sprite) {
  if (
    (!sprite.cx || !sprite.cx.length) &&
    (!sprite.cxc || !Object.keys(sprite.cxc).length)
  ) {
...
function cosmetics_logic(sprite) {
  ...
  if (
    sprite.slots &&
    sprite.slots.helmet &&
    sprite.slots.helmet.name.startsWith("ghat")
  ) {
    var found = false;
    for (var slot in sprite.slots)
      if (sprite.slots[slot] && sprite.slots[slot].giveaway) found = true;
  ...
  if (sprite.rip) {
    ...
    for (var place in sprite.cx) {
      ...
      if (
        place == "upper" &&
        (body_type == "full" || SSU[cid] != SSU[sprite.skin])
      )
        continue;
      if (body_type == "armor" && place == "head")
        needed.push(
          {
            small:
              (G.cosmetics.head[cid] && G.cosmetics.head[cid][0]) || "sskin1a",
            normal:
              (G.cosmetics.head[cid] && G.cosmetics.head[cid][1]) || "mskin1a",
            tall:
              (G.cosmetics.head[cid] && G.cosmetics.head[cid][1]) || "mskin1a",
            large:
              (G.cosmetics.head[cid] && G.cosmetics.head[cid][2]) || "lskin1a",
          }[SS[sprite.skin]],
        );
      needed.push(cid);
    }
    if (
      ...
      G.positions[
        G.items[sprite.slots.mainhand.name].skin_c ||
          G.items[sprite.slots.mainhand.name].skin
      ]
    ) {
    ...
    if (
      ...
      G.positions[
        G.items[sprite.slots.offhand.name].skin_c ||
          G.items[sprite.slots.offhand.name].skin
      ]
    ) {
  ...
  var head_dh =
    (G.cosmetics.head[sprite.cx.head] && G.cosmetics.head[sprite.cx.head][3]) ||
    0;
  var hair_dy =
    (G.cosmetics.hair[sprite.cx.hair] && G.cosmetics.hair[sprite.cx.hair][0]) ||
    0;
  hair_dy += head_dh;
  var hair_dh =
    (G.cosmetics.hair[sprite.cx.hair] && G.cosmetics.hair[sprite.cx.hair][1]) ||
    0;
  ...
  if (sprite.rip) {
    var g_height =
      G.cosmetics.gravestone[sprite.cx.gravestone || "gravestone"] || 19;
    head_dy = g_height - 23;
    hair_dy = 0;
  }
  ...
  needed.forEach(function (cid) {
    ...
    if (!sprite.cxc[cid]) {
      ...
      } else if (T[cid] == "armor") // for "upper"
      {
        ...
        var glow_distance =
          8 + (min(item.level, 13) + [1, 1.5, 2, 2, 2][gr] - 10) * 3;
        var glow_strength =
          4 + (min(item.level, 13) + [1.5, 1.75, 2, 2, 2][gr] - 10) * 2;
    ...
    if (c.stype == "copy") {
      ...
      if (sprite.j !== undefined)
        c.texture = textures[sprite.skin][sprite.i][sprite.j]; // for _clone
      c.zy = 0;
    } else if (c.stype == "bg") {
      c.zy = -100 * ZEPS;
    } else if (c.stype == "head") {
      var covers =
        G.cosmetics.prop[sprite.skin] &&
        G.cosmetics.prop[sprite.skin].includes("covers");
      ...
      c.y_disp =
        -(G.cosmetics.default_hat_place + head_dy + hair_dh + hat_dy) + head_y;
      var tilt = 0;
      c.zy = 3 * ZEPS;
      if (sprite.i == 0) c.y = c.y_disp + 1;
      else if (sprite.i == 1) c.y = c.y_disp;
...
function add_chest(data) {
  ...
  chest
    .on("mousedown", chest_click)
    .on("touchstart", chest_click)
    .on("rightdown", chest_click);
...
function add_machine(machine) {
  ...
  sprite
    .on("mousedown", machine_click)
    .on("touchstart", machine_click)
    .on("rightdown", machine_click);
...
function add_door(door) {
  ...
  function door_right_click(event) {
    ...
    if (door[7] == "key") {
      ...
      setTimeout(function () {
        show_confirm(
          "Enter " + G.maps[door[4]].name + "? [Consumes a key!]",
          ["#D06631", "Yes"],
          "No!",
          function () {
...
function add_quirk(quirk) {
  ...
  if (
    is_mobile ||
    in_arr(quirk[4], ["upgrade", "compound", "invisible_statue"])
  )
    sprite
      .on("mousedown", quirk_right_click)
      .on("touchstart", quirk_right_click);
...
function create_map() {
  ...
  for (var i = 0; i < GEO.tiles.length; i++) {
    ...
    ) // otherwise the map was processed before! [29/08/16]
    {
      ...
      if (!cached_map)
        tile_sprites[current_map][i][sprite_last[current_map][i]] =
          new_map_tile(tile_textures[current_map][i]); // new - cache one in case it increases performance [03/09/16]
    }
  }
  ...
  if (cached_map) {
    ...
    for (var last_water_frame = 0; last_water_frame < 3; last_water_frame++) {
      ...
      for (var i = 0; i < GEO.placements.length; i++) {
        ...
        if (tile[3] === undefined || tile[3] === null) {
          var def = GEO.tiles[tile[0]],
            ...
            h = def[4];
          if (
            sprite_last[current_map][tile[0]] >=
            tile_sprites[current_map][tile[0]].length
          )
            tile_sprites[current_map][tile[0]][
              sprite_last[current_map][tile[0]]
            ] = new_map_tile(tile_textures[current_map][tile[0]]);
          var entity =
            tile_sprites[current_map][tile[0]][
              sprite_last[current_map][tile[0]]++
            ]; // Sprite to add to the stage individually
          if (entity.textures)
          ...
          if (
            abs(
              ((tile[3] - tile[1]) / w + 1) * ((tile[4] - tile[2]) / h + 1),
            ) >= 20
            ...
            if (tile_textures[current_map][tile[0]].length > 1)
              ...
              texture =
                tile_textures[current_map][tile[0]][
                  last_water_frame % tile_textures[current_map][tile[0]].length
                ];
            var entity = new PIXI.extras.TilingSprite(
              texture,
              tile[3] - tile[1] + w,
              tile[4] - tile[2] + h,
            );
      ...
      renderer.render(
        tile_containers[last_water_frame],
        rtextures[last_water_frame],
      );
...
function retile_the_map() {
  ...
  if (
    !(
      ...
      abs(map.last_max_y - max_y) >= border ||
      abs(map.last_max_x - max_x) >= border
    )
  ) {
    if (last_water_frame != water_frame()) {
      last_water_frame = water_frame();
      for (var i = 0; i < water_tiles.length; i++)
        water_tiles[i].texture =
          water_tiles[i].textures[
            last_water_frame % water_tiles[i].textures.length
          ];
  ...
  if (GEO["default"] && !mdraw_tiling_sprites) {
    ...
    for (var x = min_x; x <= max_x + 10; x += def[3])
      for (var y = min_y; y <= max_y + 10; y += def[4]) {
        ...
        if (entity.textures)
          ((entity.texture = entity.textures[last_water_frame]),
            water_tiles.push(entity));
        entity.x = ex * def[3];
        entity.y = ey * def[4];
        if (mdraw_mode != "redraw")
          ((entity.parentGroup = entity.displayGroup = map_layer),
            (entity.zOrder = 0));
        entity.tid = tid;
        map.addChild(entity);
  ...
  for (var i = 0; i < GEO.placements.length; i++) {
    ...
    if (is_nun(tile[3])) {
      ...
      if (
        ...
        ((tile_sprites[current_map][tile[0]][
          ...
          s++);
      var entity =
        tile_sprites[current_map][tile[0]][sprite_last[current_map][tile[0]]++]; // Sprite to add to the stage individually
      if (entity.textures)
        ((entity.texture = entity.textures[last_water_frame]),
          water_tiles.push(entity));
      entity.x = tile[1];
    ...
    } else // #GTODO: Improve this routine, a lot [07/07/16]
    {
      ...
      if (
        ...
        (mdraw_tiling_sprites &&
          abs(((tile[3] - tile[1]) / w + 1) * ((tile[4] - tile[2]) / h + 1)) <
            8)
      ) {
        for (var x = tile[1]; x <= tile[3]; x += w) {
...
function calculate_fps() {
  ...
  if (mssince(last_count) >= frequency)
    ((last_count = new Date()),
      (fps = (frames - last_frame) * (1000.0 / frequency)),
      (last_frame = frames));
  fps_counter.text = "" + round(fps);
  fps_counter.position.set(
    width - ((inside == "com" && 5) || 335),
    height - ((inside == "com" && 5) || 0),
  );
}
    ...
    for (name in G.sprites) {
      ...
      for (var i = 0; i < matrix.length; i++)
        for (var j = 0; j < matrix[i].length; j++) {
          ...
          if (
            G.cosmetics.prop[name] &&
            G.cosmetics.prop[name].includes("slender")
          )
            SSU[name] += "slender";
        }
    }
